...
function GalleryCard({ item }: { item: (typeof galleryItems)[0] }) {
  ...
  return (
    <div className={`relative overflow-hidden group cursor-pointer ${item.colSpan}`} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <Image src={item.src} alt={item.alt} fill className={`object-cover transition-transform duration-700 ${hovered ? "scale-105" : "scale-100"}`} sizes="(max-width: 768px) 100vw, 33vw" />
      ...
      {item.type === "video" && (
        <div className="absolute top-3 right-3 bg-[#C8A882] px-2 py-0.5">
          <span className="text-[8px] tracking-[0.1em] uppercase text-white">Videó</span>
        </div>
      )}
    </div>
  );
...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    async function initGSAP() {
      ...
      ctx = gsap.context(() => {
        // ── Stats – start "top 95%" = nincs pattanás ──
        gsap.from(".stat-item", {
          opacity: 0, y: 14, stagger: 0.08, duration: 0.6, ease: "power2.out", immediateRender: false,
          scrollTrigger: { trigger: ".stats-section", start: "top 95%" },
        });
        ...
        gsap.from(".gallery-header", {
          opacity: 0, y: 12, duration: 0.7, ease: "power2.out", immediateRender: false,
          scrollTrigger: { trigger: ".gallery-section", start: "top 95%" },
        });

        gsap.from(".gallery-item", {
          opacity: 0, y: 14, stagger: 0.05, duration: 0.6, ease: "power2.out", immediateRender: false,
          scrollTrigger: { trigger: ".gallery-section", start: "top 92%" },
        });

        // ── About ──
        gsap.from(".about-visual", {
          opacity: 0, x: -30, duration: 0.9, ease: "power2.out", immediateRender: false,
          scrollTrigger: { trigger: ".about-section", start: "top 95%" },
        });
        ...
        if (aboutTitleEl) {
          ...
          gsap.from(splitAbout.lines, {
            ...
            scrollTrigger: { trigger: ".about-section", start: "top 92%" },
          });
        }

        // ── Contact ──
        gsap.from(".contact-info-col", {
          opacity: 0, y: 16, duration: 0.7, ease: "power2.out", immediateRender: false,
          scrollTrigger: { trigger: ".contact-section", start: "top 95%" },
        });

        gsap.from(".contact-form-col", {
          opacity: 0, y: 16, duration: 0.7, delay: 0.1, ease: "power2.out", immediateRender: false,
          scrollTrigger: { trigger: ".contact-section", start: "top 95%" },
        });

        gsap.from(".cf-field-anim", {
          opacity: 0, y: 10, stagger: 0.06, duration: 0.5, ease: "power2.out", immediateRender: false,
          scrollTrigger: { trigger: ".contact-form-col", start: "top 95%" },
        });
      }, rootRef);
    }
    ...
    <div ref={rootRef} className="bg-[#FAF8F4] overflow-x-hidden">
      ...
      <section className="gallery-section py-24 bg-[#FAF8F4]">
        <div className="max-w-7xl mx-auto px-8 lg:px-16">
          ...
          <div className="gallery-item grid grid-cols-2 lg:grid-cols-4 gap-3" style={{ gridAutoRows: "220px" }}>
            {galleryItems.map((item, i) => (<GalleryCard key={i} item={item} />))}
          </div>
          <div className="mt-8 sm:hidden">
            <a href="/gallery" className="inline-flex items-center gap-3 text-[11px] tracking-[0.15em] uppercase text-[#1A1510] border-b border-[#C8A882]/40 pb-0.5">Teljes galéria →</a>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <div className="about-visual relative">
              <div className="relative aspect-[3/4] overflow-hidden border border-[#DDD5C8]">
                <Image src="https://images.unsplash.com/photo-1452587925148-ce544e77e70d?w=800&q=80" alt="OptikArt stúdió" fill className="object-cover" sizes="(max-width: 1024px) 100vw, 50vw" />
            ...
            <div className="about-text-col">
              ...
              <p className="text-[14px] font-light text-[#7A6A58] leading-[1.9] mb-4">Az OptikArt csapata több mint 8 éve alkot professzionális fotó- és videótartalmakat vállalkozásoknak, magánszemélyeknek és eseményeknek egyaránt.</p>
              <p className="text-[14px] font-light text-[#7A6A58] leading-[1.9] mb-10">Minden projekt egyedi — mi is úgy közelítünk hozzá. A részletekre való odafigyelés és a kreatív látásmód határozza meg munkánkat.</p>
        ...
        <div className="max-w-7xl mx-auto px-8 lg:px-16">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
            <div className="contact-info-col">
              ...
              <p className="text-[14px] font-light text-[#7A6A58] leading-[1.9] mb-10">Legyen szó egy nagy projektről vagy egy kis megbízásról — szívesen hallunk rólad. Írj nekünk és 24 órán belül visszajelzünk.</p>
              <div className="flex flex-col gap-4">
                {[
                  { label: "Email", value: "hello@optikart.hu", icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" className="w-4 h-4"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg> },
                  { label: "Telefon", value: "+36 30 123 4567", icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" className="w-4 h-4"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1.27h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg> },
                  { label: "Helyszín", value: "Budapest, Magyarország", icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" className="w-4 h-4"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg> },
                ].map((d) => (
                  <div key={d.label} className="flex items-center gap-4">
                    <div className="w-10 h-10 border border-[#EDE8E0] rounded-full flex items-center justify-center text-[#C8A882] shrink-0">{d.icon}</div>
                    <div>
              ...
              <form className="flex flex-col gap-6" onSubmit={(e) => e.preventDefault()}>
                <div className="grid grid-cols-2 gap-6">
                  <div className="cf-field-anim">
                    ...
                    <input type="text" placeholder="Szabó Máté" required className="w-full bg-transparent border-0 border-b border-[#EDE8E0] py-2.5 text-[14px] font-light text-[#1A1510] placeholder:text-[#C8B8A0]/60 focus:outline-none focus:border-[#C8A882] transition-colors" />
                  </div>
                  <div className="cf-field-anim">
                    <label className="block text-[10px] tracking-[0.15em] uppercase text-[#A08060] mb-2">Email</label>
                    <input type="email" placeholder="pelda@email.com" required className="w-full bg-transparent border-0 border-b border-[#EDE8E0] py-2.5 text-[14px] font-light text-[#1A1510] placeholder:text-[#C8B8A0]/60 focus:outline-none focus:border-[#C8A882] transition-colors" />
                  </div>
                </div>
                <div className="cf-field-anim">
                  <label className="block text-[10px] tracking-[0.15em] uppercase text-[#A08060] mb-2">Telefonszám</label>
                  <input type="tel" placeholder="+36 30 123 4567" className="w-full bg-transparent border-0 border-b border-[#EDE8E0] py-2.5 text-[14px] font-light text-[#1A1510] placeholder:text-[#C8B8A0]/60 focus:outline-none focus:border-[#C8A882] transition-colors" />
                </div>
                <div className="cf-field-anim">
