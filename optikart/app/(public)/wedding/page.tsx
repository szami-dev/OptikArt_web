...
// Referenciák – cseréld ki a saját képeidre
const references = [
...
const faqs = [
  {
    ...
    a: "Az Alap és Extra fotós csomagokban 10 db képet 48 órán belül átadunk, hogy minél hamarabb megoszthassátok a nagy napot. A teljes szerkesztett anyag 1 hónapon belül kerül átadásra digitálisan. A videós Extra csomagnál ez 1,5 hónap.",
  },
  {
  ...
  },
  {
    ...
    a: "A szerkesztett képek alapszintű retusálást kapnak (színkorrekció, világítás). A kidolgozott képek ennél mélyebb retusálást, bőrretusálást és speciális utómunkát kapnak — ezek a legemlékezetesebb pillanatok.",
  },
  {
    q: "Van-e lehetőség egyedi csomag összeállítására?",
    a: "Természetesen! Ha a meglévő csomagok egyike sem illik tökéletesen az elképzeléseitekhez, keressetek minket és személyre szabott ajánlatot készítünk.",
  },
  {
    q: "Milyen messzire utaztok?",
    a: "Magyarország egész területén vállalunk munkát. Budapest környékén (50 km-es körzetben) kiszállási díj nincs. Ezen túl az utazási költségeket előre egyeztetjük.",
  },
  {
...
const timeline = [
  { step: "01", title: "Kapcsolatfelvétel", desc: "Írj nekünk és 24 órán belül visszajelzünk." },
  { step: "02", title: "Egyeztetés", desc: "Megismerjük az elképzeléseiteket, átbeszéljük a részleteket." },
  { step: "03", title: "Foglalás", desc: "30% foglaló befizetésével biztosítjátok a dátumot." },
  { step: "04", title: "A nagy nap", desc: "Mi gondoskodunk arról, hogy minden pillanatot megörökítsünk." },
  { step: "05", title: "Átadás", desc: "Szerkesztett képek és videók digitálisan, határidőre." },
];
    ...
    <div className="border-b border-[#EDE8E0]">
      ...
      >
        <div className="flex items-center gap-4">
          <span className="text-[10px] tracking-[0.15em] text-[#C8A882] font-light tabular-nums">
            {String(index + 1).padStart(2, "0")}
          </span>
          <span className="text-[15px] font-light text-[#1A1510] group-hover:text-[#C8A882] transition-colors duration-200">
            {q}
          </span>
        </div>
        <div className={`w-5 h-5 border border-[#EDE8E0] flex items-center justify-center shrink-0 ml-4 transition-transform duration-300 ${open ? "rotate-45" : "rotate-0"}`}>
...
function PackageCard({ pkg }: { pkg: typeof photoPackages[0] }) {
  return (
    <div className={`relative flex flex-col border transition-all duration-300 group ${
      pkg.featured
        ? "bg-[#1A1510] border-[#1A1510] shadow-2xl scale-[1.02]"
        : "bg-white border-[#EDE8E0] hover:border-[#C8A882]/50 hover:shadow-lg"
    }`}>
      {/* Tag */}
      <div className={`absolute -top-3 left-6 px-3 py-1 text-[9px] tracking-[0.18em] uppercase ${
        pkg.featured ? "bg-[#C8A882] text-white" : "bg-white border border-[#EDE8E0] text-[#A08060]"
      }`}>
        {pkg.tag}
      </div>
      ...
      <div className="p-8 pt-10">
        {/* Header */}
        <div className="mb-6">
          <h3 className={`font-['Cormorant_Garamond'] text-[2.2rem] font-light leading-none mb-1 ${
            pkg.featured ? "text-white" : "text-[#1A1510]"
          }`}>
            {pkg.name}
          </h3>
          <div className="flex items-end gap-1 mt-3">
            <span className={`font-['Cormorant_Garamond'] text-[2.8rem] font-light leading-none ${
              pkg.featured ? "text-[#C8A882]" : "text-[#1A1510]"
            }`}>
              {pkg.price}
            </span>
        ...
        <ul className="flex flex-col gap-3 mb-6">
          {pkg.items.map((item, i) => (
            <li key={i} className="flex items-start gap-3">
              <div className={`w-1 h-1 rounded-full mt-2 shrink-0 ${pkg.featured ? "bg-[#C8A882]" : "bg-[#C8A882]"}`} />
              <span className={`text-[13px] font-light leading-relaxed ${pkg.featured ? "text-white/70" : "text-[#7A6A58]"}`}>
                {item}
              </span>
            </li>
          ))}
        ...
        {pkg.gifts.length > 0 && (
          <div className={`rounded-sm p-4 mb-6 ${pkg.featured ? "bg-white/5 border border-white/10" : "bg-[#FAF8F4] border border-[#EDE8E0]"}`}>
            <div className={`text-[9px] tracking-[0.18em] uppercase mb-3 ${pkg.featured ? "text-[#C8A882]" : "text-[#C8A882]"}`}>
              Ajándék
            </div>
            {pkg.gifts.map((g, i) => (
              <div key={i} className="flex items-center gap-2">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-3 h-3 text-[#C8A882] shrink-0">
        ...
        <Link
          href="/contact"
          className={`block w-full text-center text-[11px] tracking-[0.18em] uppercase py-4 transition-all duration-300 ${
            pkg.featured
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      ctx = gsap.context(() => {
        ...
        tl.fromTo(".w-hero-label", { opacity: 0, y: 16 }, { opacity: 1, y: 0, duration: 0.7 })
          .fromTo(".w-hero-title > *", { opacity: 0, y: 60 }, { opacity: 1, y: 0, stagger: 0.15, duration: 1.1 }, "-=0.4")
          .fromTo(".w-hero-sub", { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.7 }, "-=0.6")
          .fromTo(".w-hero-cta", { opacity: 0, y: 12 }, { opacity: 1, y: 0, duration: 0.6 }, "-=0.5")
          .fromTo(".w-hero-img", { opacity: 0, scale: 1.04 }, { opacity: 1, scale: 1, stagger: 0.1, duration: 1.2, ease: "power2.out" }, "-=0.8")
          .fromTo(".w-hero-stat", { opacity: 0, y: 16 }, { opacity: 1, y: 0, stagger: 0.08, duration: 0.5 }, "-=0.5");

        // Timeline steps
        gsap.from(".w-step", {
          opacity: 0, y: 40, stagger: 0.12, duration: 0.8, ease: "power3.out", immediateRender: false,
          scrollTrigger: { trigger: ".w-timeline", start: "top 75%", toggleActions: "play none none reverse" },
        });

        gsap.from(".w-timeline-line", {
          scaleX: 0, duration: 1.5, ease: "power2.inOut", transformOrigin: "left center", immediateRender: false,
          scrollTrigger: { trigger: ".w-timeline", start: "top 75%" },
        });

        // Referenciák
        gsap.from(".w-ref-item", {
          opacity: 0, y: 30, stagger: 0.07, duration: 0.8, ease: "power3.out", immediateRender: false,
          scrollTrigger: { trigger: ".w-references", start: "top 75%", toggleActions: "play none none reverse" },
        });

        // Pricing
        gsap.from(".w-pkg-card", {
          opacity: 0, y: 40, stagger: 0.1, duration: 0.9, ease: "power3.out", immediateRender: false,
          scrollTrigger: { trigger: ".w-pricing", start: "top 75%", toggleActions: "play none none reverse" },
        });

        // FAQ
        gsap.from(".w-faq-item", {
          opacity: 0, x: -20, stagger: 0.06, duration: 0.6, ease: "power2.out", immediateRender: false,
          scrollTrigger: { trigger: ".w-faq", start: "top 78%", toggleActions: "play none none reverse" },
        });

        // Section titles SplitText
        ...
        // Parallax a hero képeken
        gsap.to(".w-hero-parallax", {
          yPercent: -12,
          ease: "none",
          scrollTrigger: { trigger: ".w-hero-section", start: "top top", end: "bottom top", scrub: 1 },
        });
      ...
      <section id="csomagok" className="w-pricing py-28 bg-[#FAFAFA]">
        <div className="max-w-7xl mx-auto px-8 lg:px-16">
          ...
          <div className="text-center mb-16">
            ...
            <h2 className="w-split-title font-['Cormorant_Garamond'] text-[clamp(2.2rem,4vw,3.5rem)] font-light text-[#1A1510] mb-4">
              Válassz csomagot
            </h2>
            <p className="text-[13px] text-[#7A6A58] max-w-md mx-auto leading-relaxed">
              Mindkét területen három csomag közül választhatsz. Kombinálható fotós + videós csomag esetén kedvezményt adunk.
            </p>
          </div>
            ...
            <div className="flex border border-[#EDE8E0] bg-white">
              {(["photo", "video"] as const).map((tab) => (
                <button
                  ...
                  className={`px-8 py-3 text-[11px] tracking-[0.15em] uppercase transition-all duration-200 ${
                    activeTab === tab
                      ? "bg-[#1A1510] text-white"
                      : "text-[#7A6A58] hover:text-[#1A1510]"
                  }`}
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 items-start">
            {(activeTab === "photo" ? photoPackages : videoPackages).map((pkg, i) => (
              <div key={i} className="w-pkg-card">
                <PackageCard pkg={pkg} />
              </div>
            ))}
          </div>

          {/* Kombinált ajánlat banner */}
          <div className="mt-12 border border-[#C8A882]/30 bg-white p-6 lg:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            <div>
              ...
              <h3 className="font-['Cormorant_Garamond'] text-[1.6rem] font-light text-[#1A1510]">
                Fotós + videós csomag kombinálva?
              </h3>
              <p className="text-[13px] text-[#7A6A58] mt-1">Egyedi kedvezményt adunk, ha mindkét szolgáltatást nálunk foglalod.</p>
            </div>
            <Link
              href="/contact"
              className="shrink-0 border border-[#C8A882] text-[#C8A882] text-[11px] tracking-[0.15em] uppercase px-8 py-3.5 hover:bg-[#C8A882] hover:text-white transition-all duration-300"
            >
      ...
      <section className="py-28 bg-[#1A1510] relative overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto px-8 text-center z-10">
          ...
          <h2 className="font-['Cormorant_Garamond'] text-[clamp(2.5rem,5vw,4.5rem)] font-thin leading-[1] text-white mb-6">
            Tegyük<br />
            <em className="not-italic text-[#C8A882]">emlékezetessé</em><br />
            a nagy napotokat
          </h2>

          <p className="text-[14px] text-white/40 leading-[1.9] mb-10 max-w-md mx-auto">
            Korlátozott számú esküvőt vállalunk évente, hogy minden párnak maximális figyelmet szentelhessünk.
          </p>

          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              href="/contact"
              className="bg-[#C8A882] text-white text-[11px] tracking-[0.18em] uppercase px-10 py-4 hover:bg-[#B8987A] transition-colors duration-300"
            >
              Időpont egyeztetés
            </Link>
            <a
              href="tel:+36301234567"
              className="border border-white/20 text-white/60 text-[11px] tracking-[0.15em] uppercase px-8 py-4 hover:border-[#C8A882]/50 hover:text-white/80 transition-all duration-300"
            >
              +36 30 123 4567
            </a>
          </div>
