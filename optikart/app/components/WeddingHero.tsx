...
export default function WeddingHero() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      ctx = gsap.context(() => {
        ...
        .fromTo(".wh-line-left",
          { scaleY: 0 },
          { scaleY: 1, duration: 1, ease: "power3.out", transformOrigin: "top center" },
          0.4
        )
        .fromTo(".wh-eyebrow",
          { opacity: 0, x: -16 },
        ...
        .fromTo(".wh-title-line",
          { opacity: 0, y: 50, skewY: 1.5 },
          { opacity: 1, y: 0, skewY: 0, stagger: 0.12, duration: 1 },
          1.0
        )
        .fromTo(".wh-side-panel",
          { opacity: 0, x: 24 },
          { opacity: 1, x: 0, duration: 0.9 },
          1.2
        )
        .fromTo(".wh-bottom-item",
          { opacity: 0, y: 16 },
          { opacity: 1, y: 0, stagger: 0.08, duration: 0.6 },
          1.4
        );

        // Parallax – csak lefelé görgetésre, sosem pozitív y
        // A wrapper 110% magas hogy ne kukucskáljon ki fekete
        gsap.to(".wh-bg-img", {
          yPercent: 12,
          ease: "none",
          scrollTrigger: {
  ...
  return (
    ...
    >
      {/* ── Háttérkép wrapper – 110% magas hogy parallax ne mutasson feketét ── */}
      <div
      ...
      >
        ...
        {/* Felső sor */}
        <div className="flex items-start gap-5 pt-8 sm:pt-10">
          <div
            className="wh-line-left w-px bg-[#C8A882]/40 origin-top shrink-0"
            style={{ height: "72px" }}
          />
          <div className="wh-eyebrow opacity-0 flex flex-col gap-1 pt-1">
            <span className="text-[9px] tracking-[0.3em] uppercase text-[#C8A882]/70">OptikArt Studio</span>
            <span className="text-[9px] tracking-[0.3em] uppercase text-white/30">Esküvői fotó & videó</span>
          </div>
        </div>

        {/* Középső – rugalmas, mindent a közepén tart */}
          ...
          <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 items-end">
            ...
            <div className="lg:col-span-8 xl:col-span-7">
              ...
              >
                ...
                <div className="wh-title-line opacity-0 overflow-hidden">
                  <em className="block not-italic" style={{ color: "#C8A882" }}>örökre.</em>
                </div>
              </div>
              <div className="flex flex-wrap items-center gap-4 mt-6">
            <Link
              href="/contact"
              className="wh-bottom-item opacity-0 bg-white text-[#1A1510] text-[11px] tracking-[0.18em] uppercase px-7 py-3.5 hover:bg-[#C8A882] hover:text-white transition-all duration-300 whitespace-nowrap"
            >
              Időpont egyeztetés
            </Link>
            <a
              href="#csomagok"
              className="wh-bottom-item opacity-0 text-[11px] tracking-[0.14em] uppercase text-white/50 border-b border-white/20 pb-0.5 hover:text-white hover:border-white/50 transition-all duration-200 whitespace-nowrap"
            >
              Csomagok →
            </a>
          </div>
            ...
            {/* Jobb panel – mobilon a cím alá kerül */}
            <div className="wh-side-panel opacity-0 lg:col-span-4 xl:col-span-5 flex flex-col gap-5 lg:pb-1">
              ...
              <div className="flex gap-6 py-4 border-t border-white/10">
                {[{ n: "120+", l: "Esküvő" }, { n: "8 év", l: "Tapasztalat" }].map((s) => (
                  <div key={s.l}>
                    <div className="font-['Cormorant_Garamond'] text-[1.8rem] font-light text-[#C8A882] leading-none">{s.n}</div>
                    <div className="text-[8px] tracking-[0.15em] uppercase text-white/30 mt-1">{s.l}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* Alsó sor */}
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-5 pb-8 sm:pb-10">
          ...
          <div className="flex items-end gap-6 sm:gap-8">
            <div className="wh-bottom-item opacity-0 hidden sm:flex items-center gap-3">
              ...
              <div>
                <div className="text-[8px] tracking-[0.15em] uppercase text-white/25">Referencia</div>
                <div className="text-[11px] text-white/40 font-['Cormorant_Garamond'] italic">Réka & Péter · Budapest, 2024</div>
              </div>
            </div>
            <div className="wh-bottom-item opacity-0 flex flex-col items-center gap-2">
              <span className="text-[8px] tracking-[0.22em] uppercase text-white/25">Scroll</span>
      ...
      {/* Jobb oldali dekor – csak nagyobb képernyőn */}
      <div className="absolute right-6 top-1/2 -translate-y-1/2 hidden xl:flex flex-col items-center gap-3 z-[2]">
        ...
        <span className="text-[7px] tracking-[0.3em] uppercase text-white/20" style={{ writingMode: "vertical-rl" }}>
          Budapest · Hungary
        </span>
        <div className="w-1 h-1 rounded-full bg-[#C8A882]/40" />
