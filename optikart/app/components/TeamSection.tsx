    ...
    async function init() {
      ...
      ctx = gsap.context(() => {
        ...
        gsap.from(".team-divider-line", {
          ...
          immediateRender: false,
          scrollTrigger: { trigger: ".team-header", start: "top 92%" },
        });
        ...
        gsap.from(".bts-header", {
          ...
          immediateRender: false,
          scrollTrigger: { trigger: ".bts-section", start: "top 95%" },
        });

        gsap.from(".bts-grid-item", {
          opacity: 0, y: 14, stagger: 0.06, duration: 0.6, ease: "power2.out",
          immediateRender: false,
          scrollTrigger: { trigger: ".bts-section", start: "top 95%" },
        });
        ...
        team.forEach((_, i) => {
          ...
          gsap.from(card.querySelectorAll(".team-bio, .team-skill-tag"), {
            opacity: 0, y: 10, stagger: 0.05, duration: 0.6, ease: "power2.out",
            immediateRender: false,
            ...
            <div key={i} className={`team-card-${i} group relative`}>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 py-16 lg:py-20">
                ...
                <div className={`lg:col-span-5 relative flex items-start gap-6 ${i % 2 !== 0 ? "lg:order-2" : ""}`}>
                  ...
                  <div className="team-img-wrap flex-1 relative">
                    <div className="relative overflow-hidden" style={{ aspectRatio: "3/4" }}>
                      ...
                      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ background: `linear-gradient(to top, ${member.accent}15, transparent)` }} />
                    </div>
                    <div className="mt-4 flex items-center gap-2">
                      <div className="w-4 h-px" style={{ background: member.accent }} />
                      <span className="text-[9px] tracking-[0.2em] uppercase font-light" style={{ color: member.accent }}>{member.focus}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className={`lg:col-span-6 lg:col-start-7 flex flex-col justify-center pt-8 lg:pt-0 ${i % 2 !== 0 ? "lg:order-1 lg:col-start-1" : ""}`}>
                  <div className="mb-6">
                    <h3 className="team-name font-['Cormorant_Garamond'] font-light text-[#1A1510] leading-none mb-2" style={{ fontSize: "clamp(2rem, 4vw, 3.2rem)" }}>
                      {member.name}
                    </h3>
                    <span className="text-[11px] tracking-[0.18em] uppercase font-light" style={{ color: member.accent }}>{member.role}</span>
                  </div>
                  <p className="team-bio text-[14px] font-light text-[#7A6A58] leading-[1.9] mb-8 max-w-md">{member.bio}</p>
                  <div className="flex flex-wrap gap-2">
                    {member.skills.map((skill) => (
                      <span key={skill} className="team-skill-tag text-[9px] tracking-[0.1em] uppercase px-3 py-1.5 border transition-all duration-300" style={{ color: member.accent, borderColor: `${member.accent}30` }}>
                        {skill}
                      </span>
                    ))}
                  </div>
              ...
              {i < team.length - 1 && (
                <div className="team-card-line h-px origin-left" style={{ background: `linear-gradient(to right, rgba(200,168,130,0.2), rgba(200,168,130,0.05), transparent)` }} />
              )}
            </div>
          ))}
        </div>
