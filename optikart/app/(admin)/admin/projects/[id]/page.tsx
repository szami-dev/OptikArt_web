...
type Project = {
  ...
  galleries: {
    id: number; title: string | null; shareableLink: string | null; expiresAt: string | null;
    images: { id: number; fileName: string | null; filePath: string | null }[];
    imagesFull: { id: number; fileName: string | null; filePath: string | null }[];
  }[];
  messages: {
  ...
  if (!project) return (
    <div className="min-h-screen bg-[#0C0A08] flex items-center justify-center">
      <div className="text-center">
        <p className="text-[14px] text-[#5A5248] mb-4">Projekt nem található</p>
        <Link href="/admin/projects" className="text-[11px] text-[#C8A882] border-b border-[#C8A882]/30 pb-0.5">← Vissza</Link>
      </div>
    </div>
  );
            ...
            <div className="lg:col-span-2 flex flex-col gap-4">
              <div className="bg-[#0E0C0A] border border-white/[0.05] p-5">
                ...
                {editing ? (
                  ...
                  <div className="flex flex-col gap-4">
                    <div>
                      <div className="text-[10px] tracking-[0.12em] uppercase text-[#3A3530] mb-1">Név</div>
                      <div className="text-[14px] text-[#D4C4B0]">{project.name ?? "—"}</div>
                    </div>
                    <div>
                      <div className="text-[10px] tracking-[0.12em] uppercase text-[#3A3530] mb-1">Leírás</div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-[#0E0C0A] border border-white/[0.05] p-5">
                ...
                {project.users.length === 0 ? (
                  <p className="text-[12px] text-[#3A3530]">Nincs kapcsolt felhasználó</p>
                ) : project.users.map(u => (
                  <div key={u.id} className="flex flex-col gap-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <div className="text-[13px] text-[#D4C4B0]">{u.name ?? "—"}</div>
                        <div className="text-[10px] text-[#3A3530]">#{u.id}</div>
                      </div>
                    </div>
                    {[{ l: "Email", v: u.email }, { l: "Telefon", v: u.phone ?? "—" }].map(row => (
                      <div key={row.l}>
                        <div className="text-[9px] tracking-[0.12em] uppercase text-[#3A3530] mb-0.5">{row.l}</div>
                        <div className="text-[12px] text-[#5A5248]">{row.v}</div>
                      </div>
                    ))}
          ...
          <div className="relative z-10 w-full max-w-sm mx-4 bg-[#0E0C0A] border border-white/[0.08] p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-start gap-3 mb-5">
              ...
              <div>
                ...
                <div className="text-[11px] text-[#5A5248]">Ez a művelet nem visszavonható. Az összes üzenet, galéria és naptári esemény is törlődik.</div>
              </div>
            </div>
            <div className="flex gap-3">
              ...
              <button onClick={deleteProject} disabled={deleting} className="flex-1 py-2.5 bg-red-500/80 text-[11px] tracking-[0.1em] uppercase text-white hover:bg-red-500 transition-all disabled:opacity-50">
