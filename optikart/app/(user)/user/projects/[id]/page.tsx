...
type Project = {
  ...
  galleries: {
    id: number; title: string | null;
    shareableLink: string | null; expiresAt: string | null;
    images: { id: number; fileName: string | null; filePath: string | null }[];
  }[];
  messages: {
        ...
        return (
          <div key={s} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center gap-1 shrink-0">
              <div className={`w-6 h-6 flex items-center justify-center border text-[10px] transition-all ${done ? "border-current" : "border-[#EDE8E0] text-[#C8B8A0]"}`}
                style={done ? { borderColor: m.color, background: m.bg, color: m.color } : {}}>
                {done
                  ? <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-3 h-3"><polyline points="20 6 9 17 4 12"/></svg>
                  : i + 1
                }
              </div>
              <span className={`text-[9px] tracking-[0.08em] whitespace-nowrap ${active ? "font-medium" : ""}`}
                style={done ? { color: m.color } : { color: "#C8B8A0" }}>
                {m.label}
              </span>
            </div>
            {i < STATUS_STEPS.length - 1 && (
              <div className={`flex-1 h-px mx-2 mb-4 transition-all ${i < idx ? "bg-[#C8A882]/30" : "bg-[#EDE8E0]"}`} />
            )}
          </div>
        );
      })}
...
export default function UserProjectDetailPage() {
  ...
  async function sendMessage() {
    ...
    setSendingMsg(true);
    try {
      // Admin ID megkeresése – az első nem-saját résztvevő vagy fallback keresés
  ...
  if (notFound || !project) return (
    <div className="min-h-screen bg-[#FAF8F4] flex items-center justify-center px-6">
      <div className="text-center">
        ...
        <Link href="/user/projects" className="text-[11px] tracking-[0.12em] uppercase text-[#C8A882] border-b border-[#C8A882]/30 pb-0.5">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="py-5 sm:py-6">
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              ...
              <span className="text-[10px] tracking-[0.08em] px-3 py-1.5 border font-medium shrink-0"
          ...
          <div className="flex gap-0 overflow-x-auto scrollbar-none -mb-px">
            {tabs.map(t => (
              <button key={t.key} onClick={() => setTab(t.key)}
                className={`flex items-center gap-1.5 px-4 py-3 text-[11px] tracking-[0.08em] uppercase border-b-2 transition-all whitespace-nowrap ${
                  tab === t.key
                    ? "border-[#C8A882] text-[#1A1510]"
                    : "border-transparent text-[#A08060] hover:text-[#1A1510]"
                }`}>
                {t.label}
                {t.count !== undefined && t.count > 0 && (
                  <span className={`text-[9px] px-1.5 py-0.5 rounded-full ${tab === t.key ? "bg-[#C8A882]/15 text-[#C8A882]" : "bg-[#EDE8E0] text-[#A08060]"}`}>
                    {t.count}
                  </span>
                )}
                {t.key === "messages" && unreadCount > 0 && tab !== "messages" && (
        ...
        {tab === "overview" && (
          <div className="flex flex-col gap-5">
            ...
            {status === "ON_HOLD" || status === "CANCELLED" ? (
              <div className="bg-white border p-5 sm:p-6 flex items-center gap-4"
                style={{ borderColor: sm.border }}>
                <div className="w-10 h-10 flex items-center justify-center shrink-0" style={{ background: sm.bg, borderColor: sm.border }}>
                ...
                <div>
                  ...
                  <div className="text-[12px] text-[#7A6A58] mt-0.5">
                    {status === "ON_HOLD" ? "A projekt jelenleg szünetel. Vedd fel velünk a kapcsolatot a folytatáshoz." : "Ez a projekt le lett zárva."}
                  </div>
                </div>
              </div>
            ) : null}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="bg-white border border-[#EDE8E0] p-5">
                ...
                <div className="flex flex-col gap-4">
                  {[
                    ...
                    { l: "Létrehozva", v: new Date(project.createdAt).toLocaleDateString("hu-HU") },
                  ].map(row => (
                    <div key={row.l}>
              ...
              {/* Leírás */}
              <div className="bg-white border border-[#EDE8E0] p-5">
                <div className="text-[10px] tracking-[0.16em] uppercase text-[#A08060] mb-4">Leírás / megjegyzések</div>
          ...
          <div className="flex flex-col gap-4" style={{ maxWidth: "640px" }}>
            <div className="bg-white border border-[#EDE8E0] flex flex-col overflow-hidden" style={{ minHeight: "360px", maxHeight: "520px" }}>
                      ...
                      <div key={msg.id} className={`flex flex-col gap-1 ${isMe ? "items-end" : "items-start"}`}>
                        ...
                        <div className={`max-w-[85%] px-4 py-3 text-[13px] leading-relaxed ${
                          isMe
                            ...
                            : "bg-[#FAF8F4] border border-[#EDE8E0] text-[#1A1510]"
                        }`}>
                          {msg.content}
                        </div>
              ...
              <div className="border-t border-[#EDE8E0] p-4 flex gap-3">
                ...
                >
                  {sendingMsg
                    ? <span className="flex items-center gap-2"><svg className="animate-spin w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" strokeOpacity="0.25"/><path d="M12 2a10 10 0 0 1 10 10" strokeLinecap="round"/></svg></span>
                    : <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-4 h-4"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
                  }
                </button>
              </div>
            </div>
            ...
            {project.galleries.length === 0 ? (
              ...
              project.galleries.map(gallery => (
                <div key={gallery.id} className="bg-white border border-[#EDE8E0] overflow-hidden">
                  <div className="px-5 sm:px-6 py-4 border-b border-[#EDE8E0] flex items-center justify-between gap-4">
                    <div>
                      <div className="text-[13px] font-medium text-[#1A1510]">{gallery.title ?? "Galéria"}</div>
                      <div className="text-[11px] text-[#A08060] mt-0.5">
                        {gallery.images.length} kép
                        {gallery.expiresAt && ` · Elérhető: ${new Date(gallery.expiresAt).toLocaleDateString("hu-HU")}-ig`}
                      </div>
                    </div>
                  ...
                  {gallery.images.length > 0 ? (
                      ...
                      <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-2">
                        {gallery.images.slice(0, 12).map(img => (
                          <div key={img.id} className="relative bg-[#FAF8F4] overflow-hidden border border-[#EDE8E0]" style={{ aspectRatio: "1/1" }}>
                            {img.filePath ? (
                              <Image src={img.filePath} alt={img.fileName ?? ""} fill className="object-cover" sizes="120px" />
                            ) : (
                              <div className="w-full h-full flex items-center justify-center">
                                <svg viewBox="0 0 24 24" fill="none" stroke="#EDE8E0" strokeWidth="1" className="w-5 h-5"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                              </div>
                            )}
                          </div>
                        ))}
                        {gallery.images.length > 12 && (
                          <div className="relative bg-[#F5EFE6] border border-[#EDE8E0] flex items-center justify-center" style={{ aspectRatio: "1/1" }}>
                            <span className="text-[11px] text-[#A08060]">+{gallery.images.length - 12}</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="px-5 py-8 text-center">
                      <p className="text-[12px] text-[#C8B8A0]">Nincs feltöltött kép ebben a galériában.</p>
                    </div>
                  )}
                </div>
              ))
                  ...
                  <div key={ev.id} className="bg-white border border-[#EDE8E0] p-4 sm:p-5 flex items-start gap-4">
                    ...
                    {start && (
                      <div className="shrink-0 w-12 text-center border border-[#EDE8E0] py-1.5">
                        <div className="text-[9px] tracking-[0.1em] uppercase text-[#A08060]">
                          {start.toLocaleDateString("hu-HU", { month: "short" })}
                        </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[11px] text-[#A08060]">
                        {ev.wholeDay ? "Egész napos esemény" : [
