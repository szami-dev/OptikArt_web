...
function EventDateHero({ eventDate, calendarEvents }: { eventDate: string | null; calendarEvents: Project["calendarEvents"] }) {
  const now = new Date();
    ...
    <div className="flex flex-col gap-3">
      {primary && (
        <div className="bg-[#1A1510] relative overflow-hidden">
          <div className="absolute inset-0 opacity-[0.06]" style={{backgroundImage:"linear-gradient(#C8A882 1px,transparent 1px),linear-gradient(90deg,#C8A882 1px,transparent 1px)",backgroundSize:"40px 40px"}}/>
          ...
          <div className="relative flex flex-col sm:flex-row sm:items-center gap-5 p-5 sm:p-7">
            <div className="flex items-center gap-5">
              <div className="flex flex-col items-center border border-[#C8A882]/25 bg-[#C8A882]/8 px-5 py-3 min-w-[76px] shrink-0">
                <span className="text-[9px] tracking-[0.18em] uppercase text-[#C8A882]/50">{primary.getFullYear()}</span>
                <span className="font-['Cormorant_Garamond'] text-[4rem] font-light text-white leading-none">{primary.getDate()}</span>
                <span className="text-[11px] tracking-[0.08em] uppercase text-[#C8A882]">{HU_MONTHS[primary.getMonth()]}</span>
              </div>
              <div className="flex flex-col gap-1.5">
                <span className="text-[8px] tracking-[0.18em] uppercase text-[#C8A882]/30 border border-[#C8A882]/20 px-1.5 py-0.5 self-start">✦ Fotózás napja</span>
                <span className="text-[9px] tracking-[0.2em] uppercase text-[#C8A882]/40">{HU_DAYS[primary.getDay()]}</span>
                <span className="font-['Cormorant_Garamond'] text-[1.3rem] font-light text-white leading-tight">{primary.toLocaleDateString("hu-HU",{month:"long",day:"numeric"})}</span>
              </div>
            </div>
            <div className="sm:ml-auto shrink-0 sm:text-right">
              {isPrimaryToday ? (
                ...
                <div>
                  ...
                  <div className="font-['Cormorant_Garamond'] text-[1.8rem] font-light text-white/30 leading-none">{Math.abs(daysUntilPrimary!)} napja</div>
                </div>
              ) : (
                <div>
      ...
      {extraEvents.length>0 && (
        <div className="bg-white border border-[#EDE8E0] p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[9px] tracking-[0.18em] uppercase text-[#A08060]">Kapcsolódó időpontok</span>
          </div>
          <div className="flex flex-col gap-0">
            {extraEvents.map(ev => {
              ...
              return (
                <div key={ev.id} className="flex items-center gap-3 py-2.5 border-b border-[#F0EBE3] last:border-b-0">
                  <div className="w-9 h-9 border border-[#EDE8E0] flex flex-col items-center justify-center shrink-0 bg-[#FAF8F4]">
                    <span className="text-[7px] text-[#A08060] uppercase leading-none">{HU_MONTHS_SH[d.getMonth()]}</span>
                    <span className="font-['Cormorant_Garamond'] text-[1.1rem] leading-none text-[#1A1510]">{d.getDate()}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-[12px] text-[#1A1510] truncate">{ev.title??"Egyeztetés"}</div>
                    <div className="text-[10px] text-[#A08060]">{ev.wholeDay?"Egész napos":d.toLocaleTimeString("hu-HU",{hour:"2-digit",minute:"2-digit"})}</div>
                  </div>
                  <div className="text-right shrink-0">
                    {isToday ? <span className="text-[10px] text-green-600 flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"/>Ma</span>
                    : isPast  ? <span className="text-[10px] text-[#C8B8A0]">Volt</span>
                    :            <span className="text-[11px] text-[#A08060]">{days} nap</span>}
                  </div>
                </div>
              );
            })}
...
export default function UserProjectDetailPage() {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const res = await fetch(`/api/projects/${id}/messages`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:msgContent.trim(),receiverId:admin.id})});
      if (!res.ok) throw new Error();
  ...
  if (loading) return <div className="min-h-screen bg-[#FAF8F4] flex items-center justify-center"><div className="w-4 h-4 border-2 border-[#C8A882]/30 border-t-[#C8A882] rounded-full animate-spin"/></div>;
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F4]">
          ...
          <div className="py-5 sm:py-6">
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div className="min-w-0">
                <h1 className="font-['Cormorant_Garamond'] text-[1.6rem] sm:text-[2rem] font-light text-[#1A1510] leading-tight mb-1 truncate">{project.name??"Névtelen projekt"}</h1>
                <div className="flex items-center gap-2 flex-wrap text-[11px] text-[#A08060]">
                  ...
                  <span>·</span><span>{new Date(project.createdAt).toLocaleDateString("hu-HU")}</span>
                  {project.eventDate&&<><span>·</span><span className="text-[#C8A882]">✦ {new Date(project.eventDate).toLocaleDateString("hu-HU")}</span></>}
                </div>
              </div>
              <div className="flex items-center gap-2 flex-wrap shrink-0">
                <span className="text-[10px] tracking-[0.08em] px-3 py-1.5 border font-medium" style={{color:sm.color,background:sm.bg,borderColor:sm.border}}>{sm.label}</span>
                {pm&&<span className="text-[10px] tracking-[0.08em] px-3 py-1.5 border font-medium" style={{color:pm.color,background:pm.bg,borderColor:pm.border}}>{pm.label}</span>}
              </div>
            </div>
          </div>
          <div className="flex gap-0 overflow-x-auto scrollbar-none -mb-px">
            {tabs.map(t=>(
              <button key={t.key} onClick={()=>setTab(t.key)}
                ...
                {t.label}
                {t.count!==undefined&&t.count>0&&<span className={`text-[9px] px-1.5 py-0.5 rounded-full ${tab===t.key?"bg-[#C8A882]/15 text-[#C8A882]":"bg-[#EDE8E0] text-[#A08060]"}`}>{t.count}</span>}
      ...
      <div className="max-w-4xl mx-auto px-5 sm:px-8 lg:px-12 py-6 sm:py-8">
        {tab==="overview" && (
          <div className="flex flex-col gap-5">
            <EventDateHero eventDate={project.eventDate} calendarEvents={project.calendarEvents}/>
              ...
              <div className="bg-white border p-5 sm:p-6" style={{borderColor:pm?pm.border:"#EDE8E0"}}>
                ...
                <div className="flex flex-col sm:flex-row sm:items-start gap-5">
                  {project.totalPrice!=null&&(
                    <div className="flex-1">
                      ...
                      <div className="font-['Cormorant_Garamond'] text-[2rem] font-light text-[#1A1510] leading-none">{project.totalPrice.toLocaleString("hu-HU")} Ft</div>
                    </div>
                  )}
                  {pm&&(
                    <div className="flex-1">
                      <div className="text-[9px] tracking-[0.14em] uppercase text-[#C8B8A0] mb-2">Fizetési státusz</div>
                      <span className="inline-flex items-center gap-2 text-[11px] px-3 py-1.5 border font-medium mb-3" style={{color:pm.color,background:pm.bg,borderColor:pm.border}}>
                ...
                {(ps==="PENDING"||ps==="OVERDUE")&&(
                  <div className="mt-4 pt-4 border-t border-[#EDE8E0]">
                    <button onClick={()=>setTab("messages")} className="inline-flex items-center gap-2 text-[11px] tracking-[0.1em] uppercase text-[#1A1510] border border-[#EDE8E0] px-4 py-2 hover:border-[#C8A882]/50 hover:text-[#C8A882] transition-all">Kérdésem van a fizetéssel kapcsolatban →</button>
                  </div>
                )}
              </div>
            )}
              ...
              <div className="bg-white border border-[#EDE8E0] p-5">
                ...
                <div className="flex flex-col gap-4">
                  {[{l:"Típus",v:project.type?.name??"—"},{l:"Csomag",v:project.category?.name??"—"},{l:"Létrehozva",v:new Date(project.createdAt).toLocaleDateString("hu-HU")}].map(row=>(
                    <div key={row.l}><div className="text-[9px] tracking-[0.14em] uppercase text-[#C8B8A0] mb-0.5">{row.l}</div><div className="text-[13px] text-[#1A1510]">{row.v}</div></div>
                  ))}
                  {project.category?.bulletPoints&&project.category.bulletPoints.length>0&&(
                    <div>
                      ...
                      {project.category.bulletPoints.map(bp=>(
                        <div key={bp.id} className="flex items-center gap-2 mb-1"><span className="w-1 h-1 rounded-full bg-[#C8A882]/60 shrink-0"/><span className="text-[12px] text-[#7A6A58]">{bp.title}</span></div>
                      ))}
                    </div>
                  )}
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {([
                ...
                <button key={item.tab} onClick={()=>setTab(item.tab)} className="group bg-white border border-[#EDE8E0] p-4 text-left hover:border-[#C8A882]/40 transition-all">
            ...
            <div className="bg-[#F5EFE6] border border-[#EDE8E0] p-5 flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="flex-1"><div className="text-[11px] font-medium text-[#1A1510] mb-1">Kérdésed van?</div><p className="text-[12px] text-[#7A6A58]">Írj nekünk az üzenetek fülön, és 24 órán belül válaszolunk.</p></div>
              <button onClick={()=>setTab("messages")} className="shrink-0 flex items-center gap-2 bg-[#1A1510] text-white text-[11px] tracking-[0.12em] uppercase px-5 py-2.5 hover:bg-[#C8A882] transition-all whitespace-nowrap">
          ...
          <div className="flex flex-col gap-4" style={{maxWidth:"640px"}}>
            <div className="bg-white border border-[#EDE8E0] flex flex-col overflow-hidden" style={{minHeight:"360px",maxHeight:"520px"}}>
                    ...
                    <div key={msg.id} className={`flex flex-col gap-1 ${isMe?"items-end":"items-start"}`}>
                      {!isMe&&(
                        <div className="flex items-center gap-1.5 mb-0.5">
                          <div className="w-5 h-5 bg-[#1A1510] flex items-center justify-center"><span className="text-[8px] text-white font-medium">{msg.sender.name?.charAt(0).toUpperCase()??"A"}</span></div>
                      ...
                      <div className={`max-w-[85%] px-4 py-3 text-[13px] leading-relaxed ${isMe?"bg-[#1A1510] text-white":"bg-[#FAF8F4] border border-[#EDE8E0] text-[#1A1510]"}`}>{msg.content}</div>
                      <span className="text-[9px] text-[#C8B8A0]">{new Date(msg.createdAt).toLocaleString("hu-HU",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}</span>
                    </div>
                  );
                })}
              ...
              <div className="border-t border-[#EDE8E0] p-4 flex gap-3">
                ...
                <button onClick={sendMessage} disabled={sendingMsg||!msgContent.trim()} className="px-5 py-2.5 bg-[#1A1510] text-[11px] tracking-[0.1em] uppercase text-white hover:bg-[#C8A882] transition-all disabled:opacity-40">
            ...
            {project.galleries.length===0 ? (
              ...
              <div key={gallery.id} className="bg-white border border-[#EDE8E0] overflow-hidden">
                <div className="px-6 py-5 border-b border-[#EDE8E0] flex items-center justify-between bg-[#FAF8F4]/50">
                  <div>
                    <div className="flex items-center gap-2 mb-1"><div className="w-4 h-px bg-[#C8A882]"/><span className="text-[9px] tracking-[0.2em] uppercase text-[#A08060]">Átadott anyag</span></div>
                ...
                <div className="p-4 sm:p-6">
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                    {gallery.images.map((img,iIdx)=>(
                      <div key={img.id} className="group relative aspect-[4/3] bg-[#F0EBE3] overflow-hidden cursor-pointer" onClick={()=>setLightbox(iIdx)}>
                        {(img as any).thumbnailUrl ? (
                          <img src={(img as any).thumbnailUrl} alt={img.fileName??""} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"/>
                        ) : (
                          <div className="flex items-center justify-center h-full text-[10px] text-[#C8B8A0]">Betöltés...</div>
                        )}
              ...
              return (
                <div key={ev.id} className="bg-white border border-[#EDE8E0] p-4 sm:p-5 flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] text-[#A08060]">{ev.wholeDay?"Egész napos esemény":[start&&start.toLocaleTimeString("hu-HU",{hour:"2-digit",minute:"2-digit"}),ev.endTime&&`– ${new Date(ev.endTime).toLocaleTimeString("hu-HU",{hour:"2-digit",minute:"2-digit"})}`].filter(Boolean).join(" ")}</div>
                  </div>
                </div>
              );
            })}
      ...
      {lightbox!==null&&project?.galleries[0]?.images&&(
        <div className="fixed inset-0 z-[500] bg-black/95 backdrop-blur-sm flex items-center justify-center p-4 select-none" onClick={()=>setLightbox(null)}>
          <button className="absolute top-6 right-6 z-[510] text-white/50 hover:text-white transition-colors p-2" onClick={()=>setLightbox(null)}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-8 h-8"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>
          {lightbox>0&&(
            <button className="absolute left-4 z-[510] w-16 h-16 flex items-center justify-center text-white/40 hover:text-white bg-white/5 hover:bg-white/10 rounded-full transition-all" onClick={e=>{e.stopPropagation();setLightbox(lightbox-1);}}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-10 h-10"><polyline points="15 18 9 12 15 6"/></svg>
            </button>
          )}
          {lightbox<project.galleries[0].images.length-1&&(
            <button className="absolute right-4 z-[510] w-16 h-16 flex items-center justify-center text-white/40 hover:text-white bg-white/5 hover:bg-white/10 rounded-full transition-all" onClick={e=>{e.stopPropagation();setLightbox(lightbox+1);}}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-10 h-10"><polyline points="9 18 15 12 9 6"/></svg>
            </button>
          )}
          <div className="relative max-w-5xl w-full h-full flex flex-col items-center justify-center" onClick={e=>e.stopPropagation()}>
            <img key={lightbox} src={(project.galleries[0].images[lightbox] as any).previewUrl} alt="Nagyított nézet" className="max-h-[85vh] max-w-full object-contain shadow-2xl"/>
            <div className="absolute bottom-[-40px] text-white/60 text-[12px] tracking-widest uppercase">{lightbox+1} / {project.galleries[0].images.length}</div>
          </div>
        </div>
      )}
    </div>
