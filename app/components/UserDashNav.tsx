  ...
  function SidebarContent({ isMobile = false }: { isMobile?: boolean }) {
    ...
    return (
      <>
        {/* Header */}
        <div
          className={`flex items-center border-b border-black/[0.06] min-h-[68px] py-[22px] transition-all duration-[350ms] ${isCollapsed ? "justify-center px-0" : "justify-between px-5"}`}
        >
          {isCollapsed ? (
            <Link
              href="/"
          ...
          {isMobile && (
            <button
              ...
              className="w-7 h-7 rounded-md border border-black/[0.08] flex items-center justify-center text-[#9A9088] hover:bg-black/[0.04] hover:text-[#3A3530] transition-all"
            >
              <svg
                viewBox="0 0 24 24"
                fill="none"
        ...
        >
          ...
          <div
            className={`h-px bg-black/[0.06] my-1 ${isCollapsed ? "mx-3" : "mx-2.5"}`}
          />
        ...
        {/* Bottom */}
        <div
          className={`pb-4 pt-2.5 border-t border-black/[0.06] flex flex-col gap-0.5 ${isCollapsed ? "px-0 items-center" : "px-2.5"}`}
        >
          {isCollapsed && (
            <button
          ...
          <Link
            href="/user/profile"
            className={`flex items-center rounded-md hover:bg-black/[0.04] transition-colors duration-200 group relative ${isCollapsed ? "justify-center w-10 h-10 mx-auto" : "gap-2.5 px-3 py-2.5"}`}
          >
            ...
            {!isCollapsed && (
              <div className="flex-1 overflow-hidden min-w-0">
                <div className="text-[13px] text-[#2A2520] truncate">
                  {session?.user?.name ?? "Felhasználó"}
                </div>
                <div className="text-[10px] tracking-[0.08em] uppercase text-[#B8B0A8]">
                  Felhasználó
                </div>
              </div>
            )}
          ...
          <button
            ...
            className={`flex items-center rounded-md text-[#9A9088] text-[13px] whitespace-nowrap hover:bg-red-500/[0.06] hover:text-red-500 transition-all duration-200 w-full group relative ${isCollapsed ? "justify-center w-10 h-10 mx-auto" : "gap-3 px-3 py-2.5"}`}
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-14 z-40 flex items-center justify-between px-4 bg-[#FAF8F5] border-b border-black/[0.07]">
        ...
        <button
          ...
          className="w-9 h-9 border border-black/[0.08] flex items-center justify-center text-[#9A9088] hover:text-[#3A3530] hover:border-black/[0.14] transition-all"
        >
          <svg
            viewBox="0 0 24 24"
            fill="none"
      ...
      {/* Mobil overlay */}
      <div
        className={`lg:hidden fixed inset-0 bg-black/40 backdrop-blur-sm z-40 transition-opacity duration-300 ${mobileOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
        onClick={() => setMobileOpen(false)}
      />

      {/* Mobil drawer */}
      <aside
        className={`lg:hidden fixed top-0 left-0 bottom-0 w-[280px] flex flex-col bg-[#FAF8F5] border-r border-black/[0.07] z-50 transition-transform duration-[350ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
      ...
      {/* Desktop sidebar */}
      <aside
        className={`hidden lg:flex fixed top-0 left-0 bottom-0 flex-col bg-[#FAF8F5] border-r border-black/[0.07] z-50 transition-[width] duration-[350ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${collapsed ? "w-[72px]" : "w-[260px]"}`}
      >
        <SidebarContent />
      </aside>
    </>
    ...
    >
      ...
      {!collapsed && (
        <>
          ...
          {item.badge && (
            <span
              className={`inline-flex items-center justify-center min-w-[18px] h-[18px] px-1.5 rounded-full text-[10px] font-medium ${active ? "bg-white/20 text-[#F5F0E8]" : "bg-[#E8E2DA] text-[#5A5248]"}`}
            >
              {item.badge}
            </span>
          )}
      ...
      {collapsed && (
        <span className="absolute left-full ml-3 top-1/2 -translate-y-1/2 bg-[#1A1510] text-[#F5F0E8] text-[12px] px-2.5 py-1.5 rounded whitespace-nowrap opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity duration-200 z-[200]">
          {item.label}
          {item.badge && (
            <span className="ml-2 bg-white/20 text-[#F5F0E8] text-[10px] px-1.5 py-0.5 rounded-full">
              {item.badge}
            </span>
          )}
          <span className="absolute right-full top-1/2 -translate-y-1/2 border-[5px] border-transparent border-r-[#1A1510]" />
