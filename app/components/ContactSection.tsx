...
const TYPE_TO_CAT: Record<ProjectTypeId, number> = {
  eskuvo: 1, portre: 2, rendezvenyek: 3, marketing: 4, dron: 5, egyeb: 6,
};
...
const IC = {
  ...
  send:     <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-4 h-4"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>,
  check:    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-6 h-6"><polyline points="20 6 9 17 4 12"/></svg>,
  arrow:    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-4 h-4"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>,
  user:     <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.3" className="w-4 h-4"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>,
  lock:     <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.3" className="w-4 h-4"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>,
};
  ...
  return (
    <div className="flex items-center gap-1.5">
      {Array.from({ length: total }, (_, i) => i + 1).map(n => (
        <div key={n} className={`transition-all duration-300 ${n === current ? "w-5 h-1.5 bg-[#C8A882]" : n < current ? "w-1.5 h-1.5 rounded-full bg-[#C8A882]/40" : "w-1.5 h-1.5 rounded-full bg-[#EDE8E0]"}`} />
      ))}
      <span className="text-[10px] text-[#A08060] ml-1">{current}/{total}</span>
    </div>
  );
}
...
function InfoBox({ children, type = "tip" }: { children: React.ReactNode; type?: "info" | "warning" | "tip" }) {
  ...
  return (
    <div className={`${s.bg} border ${s.border} px-3 py-2.5 flex items-start gap-2 text-[11px] ${s.text} leading-relaxed`}>
...
function Btn({ children, onClick, disabled, variant = "primary", fullWidth }: {
  children: React.ReactNode; onClick?: () => void; disabled?: boolean;
  variant?: "primary" | "outline" | "ghost"; fullWidth?: boolean;
}) {
  ...
  const v = {
    ...
    outline: "px-6 border border-[#EDE8E0] text-[#A08060] hover:text-[#1A1510] hover:border-[#C8A882]/40",
    ...
    <div className="bg-white border border-[#EDE8E0] p-3 sm:p-4">
      ...
      <div className="grid grid-cols-7 mb-1">
        {["H","K","Sz","Cs","P","Szo","V"].map(d => (
          <div key={d} className="text-center text-[9px] tracking-[0.06em] uppercase text-[#C8B8A0] py-1">{d}</div>
        ))}
      </div>
...
function LocationPicker({ value, onChangeName, onChangeFee }: {
  ...
  return (
    <div ref={ref} className="flex flex-col gap-2">
      ...
      {selected && travel && (
        <div className={`border px-3 py-2.5 flex items-start gap-3 ${travel.isFree ? "bg-green-50 border-green-200" : "bg-[#FDF9F5] border-[#EDE8E0]"}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-3 mb-1">
          ...
          <div className="text-right shrink-0">
            ...
            {extraPrice > 0 && <div className="text-[9px] text-[#A08060]">+{fmt(extraPrice)} feláron</div>}
          </div>
        </div>
        {pkg.description && <p className="text-[11px] text-[#7A6A58] mb-1.5">{pkg.description}</p>}
    ...
    try {
      const res = await fetch("/api/chat/guest", {
        ...
        body: JSON.stringify({
          name,
          email,
          body: [message, phone ? `Telefon: ${phone}` : null].filter(Boolean).join("\n\n"),
        }),
      });
      if (!res.ok) {
  ...
  if (submitted) {
    return (
      <div className="text-center py-10 px-4">
        ...
        <h3 className="font-['Cormorant_Garamond'] text-[1.6rem] font-light text-[#1A1510] mb-2">Üzenet elküldve!</h3>
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-[#FAF8F4] border border-[#EDE8E0] px-4 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          <div className="text-[#C8A882]">{IC.lock}</div>
          <div>
            <div className="text-[12px] text-[#1A1510] font-medium">Van már fiókod?</div>
            <div className="text-[11px] text-[#A08060]">Bejelentkezve hozzáférsel a teljes projekt wizardhoz, csomagokhoz és árakhoz.</div>
          </div>
        </div>
        <div className="flex gap-2 shrink-0">
          <Link href="/login" className="px-4 py-2 bg-[#1A1510] text-[10px] tracking-[0.1em] uppercase text-white hover:bg-[#C8A882] transition-all whitespace-nowrap">
            Bejelentkezés
          </Link>
          <Link href="/register" className="px-4 py-2 border border-[#EDE8E0] text-[10px] tracking-[0.1em] uppercase text-[#7A6A58] hover:text-[#1A1510] transition-all whitespace-nowrap">
            Regisztráció
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Neve" required>
          <input type="text" value={name} onChange={e => setName(e.target.value)}
            placeholder="Szabó Máté" className={inputCls} />
        </Field>
      ...
      <Btn fullWidth onClick={handleSubmit} disabled={sending || !name.trim() || !email.trim() || !message.trim()}>
        {IC.send}
...
function ProjectWizard({ userName }: { userName: string }) {
  ...
  useEffect(() => {
    fetch("/api/packages").then(r => r.json()).then(d => setAllPackages(d.packages ?? [])).finally(() => setPkgLoading(false));
    fetch("/api/calendar/busy?from=" + new Date().toISOString()).then(r => r.json()).then(d => setBusyDates(d.busyDates ?? []));
  }, []);

  function getEskuvoPackages(ag: string) { return allPackages.filter(p => p.categoryId === 1 && p.subtype === ag); }
  function getPackagesForType(t: ProjectTypeId) { return allPackages.filter(p => p.categoryId === TYPE_TO_CAT[t]); }
  ...
  function handleTypeSelect(id: ProjectTypeId) {
    setTypeId(id); setEskuvoAg(null); setPortreKat(null); setSzabadteri(null); setSelectedPkg(null); setStep(2);
  }
  ...
  if (submitted) return (
    <div className="text-center py-10 px-4">
      <div className="w-12 h-12 border border-[#C8A882]/40 flex items-center justify-center mx-auto mb-4 text-[#C8A882]">{IC.check}</div>
      ...
      <h3 className="font-['Cormorant_Garamond'] text-[1.8rem] font-light text-[#1A1510] mb-2">Igénylés elküldve!</h3>
      <p className="text-[13px] text-[#7A6A58] mb-6 leading-relaxed">Hamarosan felvesszük veled a kapcsolatot.</p>
      <div className="flex flex-col sm:flex-row gap-3 justify-center">
        <Link href="/" className="px-6 py-3 border border-[#EDE8E0] text-[11px] tracking-[0.12em] uppercase text-[#7A6A58] transition-all text-center">Főoldal</Link>
        <Link href="/user/projects" className="px-6 py-3 bg-[#1A1510] text-[11px] tracking-[0.12em] uppercase text-white hover:bg-[#C8A882] transition-all text-center">Projektem →</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-0">
      ...
      {step === 1 && (
        <div className="flex flex-col gap-5">
          <div>
            <h3 className="font-['Cormorant_Garamond'] text-[1.6rem] sm:text-[2rem] font-light text-[#1A1510] mb-0.5">Milyen projektre van szükséged?</h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {([
              ...
              <button key={t.id} onClick={() => handleTypeSelect(t.id)}
                ...
                <div>
                  <div className="text-[12px] text-[#1A1510] font-medium mb-0.5">{t.name}</div>
                  <div className="text-[10px] text-[#A08060] leading-snug hidden sm:block">{t.desc}</div>
                </div>
              </button>
            ))}
          </div>
      ...
      {step === 2 && typeId === "eskuvo" && (
        <div className="flex flex-col gap-5">
          <h3 className="font-['Cormorant_Garamond'] text-[1.6rem] font-light text-[#1A1510]">Esküvői csomag</h3>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[{id:"foto",label:"Csak fotózás",icon:"camera"},{id:"video",label:"Csak videó",icon:"video"},{id:"kombinalt",label:"Fotó + Videó",icon:"sparkle"}].map(ag=>(
              <button key={ag.id} onClick={()=>{setEskuvoAg(ag.id);setSelectedPkg(null);}}
                ...
                <span className="text-[10px] text-[#1A1510] font-medium text-center leading-tight">{ag.label}</span>
              </button>
            ))}
          </div>
          {eskuvoAg && (pkgLoading
            ? <div className="flex items-center gap-2 py-3"><div className="w-4 h-4 border-2 border-[#C8A882]/30 border-t-[#C8A882] rounded-full animate-spin"/><span className="text-[12px] text-[#A08060]">Betöltés...</span></div>
            : getEskuvoPackages(eskuvoAg).length === 0
              ? <InfoBox type="info">Ehhez a kategóriához nincs még csomag beállítva. Vedd fel velünk a kapcsolatot!</InfoBox>
              : <div className="flex flex-col gap-2">{getEskuvoPackages(eskuvoAg).map(pkg=>(
                  <PackageCard key={pkg.id} pkg={pkg} selected={selectedPkg?.id===pkg.id} extraPrice={0} onClick={()=>setSelectedPkg(pkg)}/>
                ))}</div>
          )}
          <div className="flex gap-3">
            <Btn variant="outline" onClick={()=>setStep(1)}>← Vissza</Btn>
            <Btn fullWidth onClick={()=>setStep(3)} disabled={!eskuvoAg||!selectedPkg}>Tovább →</Btn>
          </div>
        </div>
      )}
      ...
      {step === 2 && typeId === "portre" && (
        <div className="flex flex-col gap-5">
          <h3 className="font-['Cormorant_Garamond'] text-[1.6rem] font-light text-[#1A1510]">Portré csomag</h3>
          <div className="grid grid-cols-3 gap-2">
            {[{id:"paros",label:"Páros / Jegyesfotózás",icon:"people"},{id:"csaladi",label:"Családi fotózás",icon:"people"},{id:"egyeni",label:"Egyéni portré",icon:"person"}].map(k=>(
              <button key={k.id} onClick={()=>{setPortreKat(k.id);setSelectedPkg(null);}}
                ...
                <span className="text-[10px] text-[#1A1510] font-medium text-center leading-tight">{k.label}</span>
              </button>
            ))}
          </div>
          {portreKat && (<>
            <div className="grid grid-cols-2 gap-2">
              {[{v:false,label:"Stúdió",desc:"Kontrollált fény, bármikor",icon:"building"},{v:true,label:"Szabadtéri",desc:`+${fmt(SZABADTERI_FELAR)} felár`,icon:"outdoor"}].map(opt=>(
            ...
            {szabadteri !== null && (pkgLoading
              ? <div className="flex items-center gap-2 py-3"><div className="w-4 h-4 border-2 border-[#C8A882]/30 border-t-[#C8A882] rounded-full animate-spin"/></div>
              : <div className="flex flex-col gap-2">{getPackagesForType("portre").map(pkg=>(
          ...
          <div className="flex gap-3">
            <Btn variant="outline" onClick={()=>setStep(1)}>← Vissza</Btn>
            <Btn fullWidth onClick={()=>setStep(3)} disabled={!portreKat||szabadteri===null||!selectedPkg}>Tovább →</Btn>
          </div>
        </div>
      )}

      {/* ═══ STEP 2/3: Részletek ═══ */}
      {((step === 2 && !hasPackages) || (step === 3 && hasPackages)) && typeId && (
        <div className="flex flex-col gap-5">
          <div>
            <h3 className="font-['Cormorant_Garamond'] text-[1.6rem] font-light text-[#1A1510] mb-0.5">Projekt részletei</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Projekt neve" required>
              <input type="text" value={projectName} onChange={e=>setProjectName(e.target.value)}
                placeholder="Pl. Szabó Esküvő 2025" className={inputCls}/>
            </Field>
            <Field label="Telefonszám">
              <input type="tel" value={phone} onChange={e=>setPhone(e.target.value)}
                placeholder="+36 30 123 4567" className={inputCls}/>
          ...
          {/* Naptár */}
          <div>
            <div className="text-[10px] tracking-[0.14em] uppercase text-[#A08060] mb-2">Kívánt időpont (opcionális)</div>
              ...
              <div className="flex flex-col gap-2">
                {preferredDate ? (
                  <div className="bg-[#F5EFE6] border border-[#EDE8E0] p-3">
                    ...
                    <button onClick={()=>setPreferredDate("")} className="mt-1.5 text-[10px] text-[#A08060] hover:text-[#C8A882] transition-colors">Törlés ×</button>
                  </div>
                ) : (
                  <div className="bg-white border border-dashed border-[#EDE8E0] p-3 text-center">
                    <div className="text-[12px] text-[#C8B8A0]">Válassz napot</div>
                    <div className="text-[10px] text-[#C8B8A0] mt-0.5">(nem kötelező)</div>
                  </div>
                )}
                {preferredDate && isTelHonap(preferredDate) && <InfoBox type="warning">Téli időpont! Dec–feb között szabadtéri fotózás korlátozott.</InfoBox>}
                {preferredDate && isOsziHonap(preferredDate) && !isTelHonap(preferredDate) && <InfoBox type="warning">Őszi időpontban az időjárás változékony.</InfoBox>}
              </div>
            </div>
          </div>
          ...
          {error && <p className="text-[11px] text-red-500">{error}</p>}
          <div className="flex gap-3">
            <Btn variant="outline" onClick={()=>setStep(hasPackages?2:1)}>← Vissza</Btn>
            <Btn fullWidth onClick={()=>handleDetailsNext(hasPackages?4:3)}>Összefoglaló →</Btn>
          </div>
        </div>
      )}
      ...
      {((step === 3 && !hasPackages) || (step === 4 && hasPackages)) && (
        <div className="flex flex-col gap-5">
          <div>
            <h3 className="font-['Cormorant_Garamond'] text-[1.6rem] font-light text-[#1A1510] mb-0.5">Összefoglaló</h3>
            <p className="text-[12px] text-[#A08060]">Ellenőrizd az adatokat</p>
          </div>
          <div className="flex flex-col gap-1.5">
            {selectedPkg && (
              <div className="bg-[#F5EFE6] border border-[#EDE8E0] p-3">
                <div className="text-[9px] tracking-[0.14em] uppercase text-[#A08060] mb-0.5">Csomag</div>
            ...
            {[
              ...
              <div key={row.label} className="bg-white border border-[#EDE8E0] px-3 py-2.5 flex items-center justify-between gap-4">
                <span className="text-[10px] tracking-[0.12em] uppercase text-[#A08060]">{row.label}</span>
                <span className="text-[12px] text-[#1A1510] text-right truncate">{row.value}</span>
              </div>
            ))}
            {totalPrice > 0 && (
              <div className="bg-white border border-[#EDE8E0] px-3 py-2.5 flex items-center justify-between">
                <span className="text-[10px] tracking-[0.12em] uppercase text-[#A08060]">Becsült összeg</span>
            ...
            <div className="bg-white border border-[#EDE8E0] px-3 py-2.5">
              <div className="text-[10px] tracking-[0.12em] uppercase text-[#A08060] mb-1">Leírás</div>
              <div className="text-[12px] text-[#7A6A58] leading-relaxed line-clamp-3">{description}</div>
            </div>
          </div>
          {error && <p className="text-[11px] text-red-500">{error}</p>}
          <div className="flex gap-3">
            <Btn variant="outline" onClick={()=>setStep(hasPackages?3:2)}>← Módosítás</Btn>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 sm:gap-20">
        ...
        <div>
          ...
          <p className="text-[13px] sm:text-[14px] font-light text-[#7A6A58] leading-[1.9] mb-8">
            Legyen szó egy nagy projektről vagy egy kis megbízásról — szívesen hallunk rólad.
            {isLoggedIn
          ...
          <div className="flex flex-col gap-4">
            {[
              ...
              <div key={d.label} className="flex items-center gap-3">
                <div className="w-9 h-9 border border-[#EDE8E0] rounded-full flex items-center justify-center text-[#C8A882] shrink-0">{d.icon}</div>
                <div>
