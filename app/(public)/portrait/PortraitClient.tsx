...
function YouTubeEmbed({ videoId, title }: { videoId: string; title: string }) {
  ...
  return (
    <div
      className="relative aspect-video bg-[#0F0D0B] overflow-hidden group cursor-pointer"
...
function PackageCard({ pkg, featured }: { pkg: Package; featured?: boolean }) {
  return (
    <div
      className={`relative flex flex-col border transition-all duration-300 h-full ${featured ? "bg-[#1A1510] border-[#1A1510] shadow-2xl" : "bg-white border-[#EDE8E0] hover:border-[#C8A882]/50 hover:shadow-md"}`}
    >
      {featured && (
      ...
      <div className="p-6 pt-8 flex flex-col flex-1">
        <h3
          className={`font-['Cormorant_Garamond'] text-[1.8rem] font-light leading-none mb-1 ${featured ? "text-white" : "text-[#1A1510]"}`}
        >
          {pkg.name}
        </h3>
        {pkg.description && (
          <p
            className={`text-[12px] mt-1.5 mb-3 leading-relaxed ${featured ? "text-white/50" : "text-[#A08060]"}`}
          >
            {pkg.description}
          </p>
        )}
        <div className="flex items-end gap-1 mb-5">
          <span
            className={`font-['Cormorant_Garamond'] text-[2.4rem] font-light leading-none ${featured ? "text-[#C8A882]" : "text-[#1A1510]"}`}
          >
            {pkg.price ? pkg.price.toLocaleString("hu-HU") : "—"}
          </span>
          {pkg.price && (
            <span
              className={`text-[11px] mb-1 ${featured ? "text-[#C8A882]/60" : "text-[#A08060]"}`}
            >
              Ft
            </span>
          )}
        ...
        <div
          className={`h-px mb-5 ${featured ? "bg-white/10" : "bg-[#EDE8E0]"}`}
        />
          ...
          <ul className="flex flex-col gap-2.5 mb-6 flex-1">
            {pkg.bulletPoints.map((bp) => (
              <li key={bp.id} className="flex items-start gap-2.5">
                ...
                <span
                  className={`text-[12px] font-light leading-relaxed ${featured ? "text-white/70" : "text-[#7A6A58]"}`}
                >
                  {bp.title}
                </span>
              </li>
        ...
        <Link
          href="/contact"
          className={`mt-auto block w-full text-center text-[11px] tracking-[0.18em] uppercase py-3.5 transition-all duration-300 ${featured ? "bg-[#C8A882] text-white hover:bg-[#B8987A]" : "border border-[#1A1510] text-[#1A1510] hover:bg-[#1A1510] hover:text-white"}`}
        >
          Időpontot kérek
        </Link>
      </div>
...
export default function PortraitClient() {
  ...
  useEffect(() => {
    fetch("/api/packages")
      ...
      .then((d) =>
        setPackages(
          (d.packages ?? []).filter(
            (p: Package) => (p.categoryId ?? p.category?.id) === 2,
          ),
        ),
      )
  ...
  function isFeatured(i: number, total: number) {
    return total === 3 && i === 1;
  }
    ...
    async function init() {
      ...
      ctx = gsap.context(() => {
        ...
        gsap.from(".ph-pkg-card", {
          ...
          ease: "power2.out",
          immediateRender: false,
          scrollTrigger: { trigger: ".ph-packages", start: "top 95%" },
        });
    ...
    <div ref={rootRef} className="bg-white overflow-x-hidden">
      ...
      >
        <div className="lg:hidden absolute inset-0 z-0">
          <Image
            src="/gallery/portrait/liliMesiFranka-web -50.jpg"
            alt="Portré"
            fill
        ...
        <div className="hidden lg:grid absolute inset-0 grid-cols-[1fr_38%_24%] grid-rows-2 gap-1.5 p-1.5">
          ...
          <div className="ph-mosaic-cell opacity-0 relative overflow-hidden">
            ...
            <div className="absolute bottom-3 left-3 z-10">
              <span className="text-[8px] tracking-[0.12em] uppercase text-white/70 bg-black/30 backdrop-blur-sm px-2 py-1">
                Páros
              </span>
            </div>
          </div>
          <div className="ph-mosaic-cell opacity-0 relative overflow-hidden">
            <Image
              src="/gallery/portrait/heniek-46.jpg"
              alt="Família"
              fill
              className="object-cover hover:scale-105 transition-transform duration-700"
              sizes="24vw"
          ...
          <div className="flex-1 flex flex-col justify-center gap-5 py-8">
            ...
            >
              <div className="ph-title-word opacity-0 overflow-hidden">
                <span className="block">Arcok,</span>
              </div>
              <div className="ph-title-word opacity-0 overflow-hidden">
                <span className="block">pillanatok,</span>
              </div>
              <div className="ph-title-word opacity-0 overflow-hidden">
            ...
            <p className="ph-hero-desc opacity-0 text-[13px] font-light text-[#7A6A58] leading-[1.9] max-w-xs">
              Páros, jegyes, családi és egyéni portré fotózás. Természetes
              fényben, valódi pillanatokból.
            </p>
          ...
          <div className="shrink-0 flex gap-6 pt-4 border-t border-[#EDE8E0]">
            {[
              ...
              <div key={s.l} className="ph-hero-stat opacity-0">
                <div className="font-['Cormorant_Garamond'] text-[1.5rem] font-light text-[#C8A882] leading-none">
                  {s.n}
                </div>
                <div className="text-[8px] tracking-[0.12em] uppercase text-[#A08060] mt-0.5">
                  {s.l}
                </div>
              </div>
            ))}
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-16">
          ...
          <div className="flex flex-col gap-16 sm:gap-24">
            {photographers.map((p, i) => (
              <div
                key={i}
                className={`ph-photo-card grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start ${i % 2 !== 0 ? "lg:[direction:rtl]" : ""}`}
              >
                <div
                  className={`lg:col-span-5 ${i % 2 !== 0 ? "lg:[direction:ltr]" : ""}`}
                >
                  <div
                    className="relative overflow-hidden border border-[#EDE8E0] mb-4"
                  ...
                  <div className="flex flex-wrap gap-2">
                    {p.specialties.map((s) => (
                      <span
                        key={s}
                        className="text-[9px] tracking-[0.1em] uppercase px-3 py-1.5 border border-[#EDE8E0] text-[#A08060]"
                      >
                        {s}
                      </span>
                    ))}
                ...
                <div
                  className={`lg:col-span-7 flex flex-col gap-6 ${i % 2 !== 0 ? "lg:[direction:ltr]" : ""}`}
                >
                  <div>
                    ...
                    <h3 className="font-['Cormorant_Garamond'] text-[clamp(2rem,4vw,3rem)] font-light text-[#1A1510] leading-none mb-1">
                      {p.name}
                    </h3>
                    <p className="text-[11px] tracking-[0.12em] uppercase text-[#A08060] mb-4">
                      {p.role}
                    </p>
                    <p className="text-[14px] font-light text-[#7A6A58] leading-[1.9] max-w-lg">
                      {p.bio}
                    </p>
                  </div>
                  <div className="flex gap-8 py-4 border-t border-b border-[#EDE8E0]">
                    {p.stats.map((s) => (
                      <div key={s.l}>
                        <div
                          className="font-['Cormorant_Garamond'] text-[1.8rem] font-light leading-none mb-0.5"
                          style={{ color: p.accent }}
                        >
                          {s.n}
                        </div>
