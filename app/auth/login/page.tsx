...
// app/(auth)/auth/login/page.tsx – frissített login 2FA redirect logikával
...
function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!exists) { setLoading(false); setErrorType("wrong_email"); return; }
    if (!isVerified) { setLoading(false); setErrorType("unverified"); return; }

    // Bejelentkezés
    const result = await signIn("credentials", { email, password, redirect: false });

    if (result?.error) { setLoading(false); setErrorType("wrong_password"); return; }
    ...
    if (role === "ADMIN") {
      if (!twoFactorEnabled) {
        // Admin, nincs 2FA → kötelező beállítani
        router.push("/auth/2fa-setup");
      } else {
        // Admin, van 2FA → kód bekérés
        router.push("/auth/2fa-verify?redirect=/admin/dashboard");
      }
      return;
    }

    if (twoFactorEnabled) {
      // User, van 2FA → kód bekérés
      const target = callbackUrl || "/user/dashboard";
      router.push(`/auth/2fa-verify?redirect=${encodeURIComponent(target)}`);
      return;
    }

    // User, nincs 2FA → egyenesen be
    ...
    <div ref={rootRef} className="flex h-screen bg-[#FAF8F4] overflow-hidden">
      ...
      <div ref={formRef} className="flex flex-col justify-center w-full md:w-2/3 px-8 sm:px-16 lg:px-28 xl:px-40 py-12 relative opacity-0">
        ...
        <div className="relative z-10 max-w-md w-full">
          ...
          <h1 className="form-anim-item font-['Cormorant_Garamond'] text-[clamp(2rem,3.5vw,3rem)] font-light leading-[1.1] text-[#1A1510] mb-3 opacity-0">Jó visszalátni</h1>
          <p className="form-anim-item text-[13px] font-light text-[#7A6A58] leading-relaxed mb-10 opacity-0">Add meg az adataidat a belépéshez.</p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            <div className="form-anim-item opacity-0">
              ...
              <input ref={emailRef} name="email" type="email" placeholder="pelda@email.com" required
            ...
            <div className="form-anim-item opacity-0">
              <div className="flex justify-between items-center mb-2">
                ...
                <a href="/auth/forgot-password" className="text-[10px] tracking-[0.08em] text-[#C8A882] hover:text-[#A08060] transition-colors">Elfelejtett jelszó?</a>
              </div>
              ...
              {errorType === "wrong_password" && (
                <p className="mt-2 text-[11px] text-red-400/80 flex items-center gap-1.5">
                  <span className="w-1 h-1 rounded-full bg-red-400/80 shrink-0 inline-block"/>
                  Helytelen jelszó.{" "}
                  <a href="/auth/forgot-password" className="underline underline-offset-2 hover:text-red-500 transition-colors">Elfelejtette?</a>
                </p>
              )}
            </div>
            ...
            {errorType === "unverified" && (
              <div className="border border-[#C8A882]/30 bg-[#C8A882]/5 px-4 py-4 flex flex-col gap-2.5">
                <p className="text-[12px] font-light text-[#7A6A58] leading-relaxed">Az email címed még nincs megerősítve.</p>
                {resendStatus === "sent" ? (
                  <p className="text-[11px] text-[#C8A882] flex items-center gap-1.5"><span className="w-1 h-1 rounded-full bg-[#C8A882] inline-block"/>Elküldtük!</p>
                ) : resendStatus === "error" ? (
