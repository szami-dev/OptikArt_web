...
// VÁLTOZÁSOK:
// 1. editEventDate state + mező a szerkesztő formban
// 2. saveEdit: elküldi az eventDate-t, és ha változott → email
// 3. AdminGalleryManager-nek átadjuk a project user adatait + onSent callback
...
> = {
  PENDING: {
    ...
    color: "#FBBF24",
    bg: "rgba(251,191,36,0.1)",
    desc: "A fizetés még nem érkezett meg.",
  },
  PAID: {
    label: "Fizetve",
    color: "#34D399",
    bg: "rgba(52,211,153,0.1)",
    desc: "A fizetés sikeresen megérkezett.",
  },
  OVERDUE: {
    label: "Lejárt",
    color: "#F87171",
    bg: "rgba(248,113,113,0.1)",
    desc: "A fizetési határidő lejárt.",
  },
  REFUNDED: {
    label: "Visszatérítve",
    color: "#A78BFA",
...
// ── Toast, EventDateCardDark – változatlanok, hagyd őket ──────
// (ezeket ne másold újra, maradjanak a meglévő verzió)
...
export default function AdminProjectDetailPage() {
  ...
  const [editEventDate, setEditEventDate] = useState(""); // ← ÚJ
  ...
  const fetchProject = useCallback(async () => {
    try {
      ...
      // ── ÚJ: eventDate beállítása (YYYY-MM-DD formátum a date inputhoz)
      setEditEventDate(
        data.project.eventDate
          ? new Date(data.project.eventDate).toISOString().slice(0, 10)
          : "",
      );
    } catch {
  ...
  // ── Mentés – eventDate + email ha változott ───────────────
  async function saveEdit() {
    setSaving(true);
    try {
      const oldDateStr = project?.eventDate
        ? new Date(project.eventDate).toISOString().slice(0, 10)
        : "";
      const dateChanged = editEventDate !== oldDateStr;
      ...
      const res = await fetch(`/api/projects/${id}`, {
        ...
        body: JSON.stringify({
          ...
          eventDate: editEventDate || null, // ← ÚJ
          // Ha a dátum változott, a backend küldi az emailt
          notifyDateChange: dateChanged && !!editEventDate,
        }),
      });
      if (!res.ok) throw new Error();
      await fetchProject();
      setEditing(false);
      setToast({ msg: "Projekt mentve", type: "success" });
    } catch {
      setToast({ msg: "Hiba a mentésnél", type: "error" });
    } finally {
      setSaving(false);
    }
  }
  ...
  async function updatePaymentStatus(paymentStatus: PaymentStatus) {
    try {
      ...
      setToast({
        msg: `Fizetési státusz: ${PAYMENT_META[paymentStatus].label}`,
        type: "success",
      });
    } catch {
      setToast({ msg: "Hiba a frissítésnél", type: "error" });
    }
  }
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setToast({ msg: "Hiba az üzenet küldésnél", type: "error" });
    } finally {
      setSendingMsg(false);
    }
  }
  ...
  async function deleteProject() {
    ...
    try {
      ...
      setToast({ msg: "Hiba a törléskor", type: "error" });
      setDeleting(false);
    }
  }
    ...
    return (
      <div className="min-h-screen bg-[#0C0A08] flex items-center justify-center">
        <div className="text-center">
          <p className="text-[14px] text-[#5A5248] mb-4">
            Projekt nem található
          </p>
          <Link
            href="/admin/projects"
            className="text-[11px] text-[#C8A882] border-b border-[#C8A882]/30 pb-0.5"
          >
            ← Vissza
          </Link>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#0C0A08] text-[#D4C4B0]">
      ...
      <div className="border-b border-white/[0.05] px-4 sm:px-6 lg:px-8 py-4">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3 min-w-0">
            <Link
              href="/admin/projects"
              className="text-[#3A3530] hover:text-[#D4C4B0] transition-colors shrink-0"
            >
              <svg
                viewBox="0 0 24 24"
                fill="none"
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="font-['Cormorant_Garamond'] text-[1.4rem] sm:text-[1.6rem] font-light text-white leading-tight truncate">
                  {project.name ?? "Névtelen projekt"}
                </h1>
                <span
                  className="text-[9px] tracking-[0.1em] uppercase px-2 py-1 border shrink-0"
                  style={{
                    color: sm.color,
                    background: sm.bg,
                ...
                {project.eventDate && (
                  <>
                    ...
                    <span className="text-[#C8A882]">
                      📅{" "}
                      {new Date(project.eventDate).toLocaleDateString("hu-HU")}
                    </span>
                  </>
                )}
            ...
            {!editing ? (
              <>
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 border border-red-500/20 text-[11px] tracking-[0.08em] uppercase text-red-400/60 hover:text-red-400 hover:border-red-500/40 transition-all"
                >
                  <svg
                    viewBox="0 0 24 24"
                    fill="none"
              ...
              <>
                <button
                  ...
                  className="px-3 py-2 border border-white/[0.08] text-[11px] text-[#5A5248] hover:text-[#D4C4B0] transition-all"
                >
                  Mégsem
                </button>
                <button
                  onClick={saveEdit}
                  disabled={saving}
                  className="px-5 py-2 bg-[#C8A882] text-[11px] tracking-[0.1em] uppercase text-[#0C0A08] font-medium hover:bg-[#D4B892] transition-all disabled:opacity-50"
                >
        ...
        <div className="flex gap-0 mt-5 border-b border-white/[0.04] -mb-4 overflow-x-auto scrollbar-none">
          {(["overview", "messages", "gallery", "calendar"] as Tab[]).map(
            (t) => {
              ...
              return (
                <button
                  key={t}
                  onClick={() => setTab(t)}
                  className={`px-4 py-2.5 text-[11px] tracking-[0.08em] uppercase border-b-2 transition-all whitespace-nowrap ${
                    tab === t
                      ? "border-[#C8A882] text-[#C8A882]"
                      : "border-transparent text-[#3A3530] hover:text-[#5A5248]"
                  }`}
                >
                  {labels[t]}
                </button>
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-6">
        ...
        {tab === "overview" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div className="lg:col-span-2 flex flex-col gap-4">
              {/* EventDateCard – változatlan, hagyd */}
              {/* (az EventDateCardDark komponenst ne töröld) */}
              ...
              <div className="bg-[#0E0C0A] border border-white/[0.05] p-5">
                <div className="text-[9px] tracking-[0.16em] uppercase text-[#3A3530] mb-4">
                  Projekt adatok
                </div>
                ...
                {editing ? (
                  <div className="flex flex-col gap-4">
                    {/* Név */}
                    <div>
                      <label className="text-[10px] tracking-[0.12em] uppercase text-[#5A5248] block mb-1.5">
                        Projekt neve
                      </label>
                      <input
                        value={editName}
                          ...
                          editEventDate !==
                            (project.eventDate
                              ? new Date(project.eventDate)
                                  ...
                                  .slice(0, 10)
                              : "") && (
                            <span className="ml-2 text-[#FBBF24] text-[9px] tracking-normal normal-case">
                              ⚠ Megváltozott – az ügyfél értesítést kap
                            </span>
                          )}
                      </label>
                      <input
                  ...
                  <div className="flex flex-col gap-4">
                    <div>
                      <div className="text-[10px] tracking-[0.12em] uppercase text-[#3A3530] mb-1">
                        Név
                      </div>
                      <div className="text-[14px] text-[#D4C4B0]">
                        {project.name ?? "—"}
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="text-[13px] text-[#5A5248] leading-relaxed whitespace-pre-wrap">
                        {project.description ?? "—"}
                      </div>
                    </div>
                    <div className="flex flex-wrap gap-6 pt-3 border-t border-white/[0.04]">
                      <div>
                        <div className="text-[9px] tracking-[0.12em] uppercase text-[#3A3530] mb-1">
                          Típus
                        </div>
                        <div className="text-[12px] text-[#D4C4B0]">
                          {project.type?.name ?? "—"}
                        </div>
                      </div>
                      <div>
                      ...
                      {project.eventDate && (
                        <div>
                          <div className="text-[9px] tracking-[0.12em] uppercase text-[#C8A882]/50 mb-1">
                            Munka dátuma
                          </div>
                          <div className="text-[12px] text-[#C8A882]">
              ...
              {project.galleries.length > 0 && (
                <div className="bg-[#0E0C0A] border border-white/[0.05] p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div className="text-[9px] tracking-[0.16em] uppercase text-[#3A3530]">
                      Galériák
                    </div>
                    <button
                      onClick={() => setTab("gallery")}
                      className="text-[10px] text-[#C8A882]/60 hover:text-[#C8A882] transition-colors"
                    >
                      Mind →
                    </button>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {project.galleries.slice(0, 3).map((g) => (
                      <div
                        key={g.id}
                        className="bg-[#141210] border border-white/[0.04] p-3"
                      >
                        <div className="text-[12px] text-[#D4C4B0] truncate mb-1">
                          {g.title ?? "Névtelen galéria"}
                        </div>
                        <div className="text-[10px] text-[#3A3530]">
                          {g.images.length + g.imagesFull.length} kép
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-[#0E0C0A] border border-white/[0.05] p-5">
                ...
                {project.users.length === 0 ? (
                  <p className="text-[12px] text-[#3A3530]">
                    Nincs kapcsolt felhasználó
                  </p>
                ) : (
                  project.users.map((u) => (
                    <div key={u.id} className="flex flex-col gap-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-9 h-9 border border-[#C8A882]/20 flex items-center justify-center font-['Cormorant_Garamond'] text-[16px] text-[#C8A882]">
                          {u.name?.charAt(0).toUpperCase() ?? "?"}
                        </div>
                        <div>
                          <div className="text-[13px] text-[#D4C4B0]">
                            {u.name ?? "—"}
                          </div>
                          <div className="text-[10px] text-[#3A3530]">
                            #{u.id}
                      ...
                      {[
                        ...
                        <div key={row.l}>
                          <div className="text-[9px] tracking-[0.12em] uppercase text-[#3A3530] mb-0.5">
                            {row.l}
                          </div>
                          <div className="text-[12px] text-[#5A5248]">
                            {row.v}
                          </div>
                        </div>
                      ))}
                      <button
                        ...
                        className="mt-1 py-2 border border-[#C8A882]/20 text-[11px] tracking-[0.1em] uppercase text-[#C8A882]/60 hover:text-[#C8A882] hover:border-[#C8A882]/40 transition-all text-center"
                      >
                        Üzenet küldése →
                      </button>
                    </div>
              ...
              {/* Projekt státusz – változatlan */}
              <div className="bg-[#0E0C0A] border border-white/[0.05] p-5">
                <div className="text-[9px] tracking-[0.16em] uppercase text-[#3A3530] mb-3">
                  Projekt státusz
                </div>
                <div className="flex flex-col gap-1.5">
                  {STATUSES.map((s) => {
                    ...
                    return (
                      <button
                        key={s}
                        onClick={() => !active && quickStatusUpdate(s)}
                        disabled={active}
                        ...
                        style={{
                          ...
                          borderColor: active ? `${v.color}40` : undefined,
                        }}
                      >
                        {active && "✓ "}
                        {v.label}
              ...
              <div className="bg-[#0E0C0A] border border-white/[0.05] p-5">
                ...
                {project.totalPrice != null ? (
                  <div className="mb-4 pb-4 border-b border-white/[0.05]">
                    ...
                    <button
                      onClick={() => setEditing(true)}
                      className="text-[10px] text-[#3A3530] hover:text-[#5A5248] mt-1 transition-colors"
                    >
                      Szerkesztés →
                    </button>
                  </div>
                  ...
                  <div className="mb-4 pb-4 border-b border-white/[0.05]">
                    ...
                    <div className="text-[12px] text-[#3A3530]">
                      Nincs megadva
                    </div>
                    <button
                      onClick={() => setEditing(true)}
                      className="text-[10px] text-[#C8A882]/50 hover:text-[#C8A882] mt-1 transition-colors"
                    >
                      + Összeg megadása
                    </button>
                  </div>
                ...
                {pm && (
                  <div
                    className="mb-3 px-3 py-2.5 border text-[11px] leading-relaxed"
                    style={{
                      borderColor: `${pm.color}30`,
                      background: pm.bg,
                      color: pm.color,
                ...
                <div className="flex flex-col gap-1.5">
                  {PAYMENT_STATUSES.map((s) => {
                    ...
                    return (
                      <button
                        key={s}
                        onClick={() => !active && updatePaymentStatus(s)}
                        disabled={active}
                        className={`py-2 px-3 text-left text-[11px] tracking-[0.08em] uppercase border transition-all ${active ? "cursor-default" : "border-white/[0.04] hover:border-current/30 cursor-pointer"}`}
                        style={{
                          color: v.color,
        ...
        {/* ── MESSAGES tab – változatlan ── */}
        {tab === "messages" && (
          <div className="max-w-2xl flex flex-col gap-4">
            <div
              className="bg-[#0E0C0A] border border-white/[0.05] flex flex-col"
            ...
            >
                      ...
                      >
                        <div
                          className={`max-w-[80%] px-4 py-2.5 text-[13px] leading-relaxed ${
                            isMe
                              ? "bg-[#C8A882]/15 border border-[#C8A882]/20 text-[#D4C4B0]"
                              : "bg-white/[0.04] border border-white/[0.06] text-[#D4C4B0]"
                          }`}
                        >
                          {msg.content}
                        </div>
              ...
              <div className="border-t border-white/[0.05] p-4 flex gap-3">
                ...
                <button
                  onClick={sendMessage}
                  disabled={
                    sendingMsg || !msgContent.trim() || !project.users[0]
                  }
        ...
        {/* ── GALLERY tab – ÚJ: client adatok + onSent callback ── */}
        {tab === "gallery" && (
          <div className="flex flex-col gap-5">
            {project.galleries?.[0] ? (
              <AdminGalleryManager
                ...
                // ← ÚJ: átadjuk az ügyfél adatait az email küldéshez
                clientEmail={project.users[0]?.email ?? null}
                clientName={project.users[0]?.name ?? null}
                initialGallery={
                  {
                    ...project.galleries[0],
                    hasPassword: !!project.galleries[0].password,
                    shareableLink: project.galleries[0].shareToken
                      ? `/gallery/${project.galleries[0].shareToken}`
                      : null,
                  } as any
                }
                onSent={() =>
                  setToast({
                    msg: "Galéria értesítő elküldve az ügyfélnek!",
                    type: "success",
                  })
                }
              />
            ...
            {project.calendarEvents.length === 0 ? (
              ...
              project.calendarEvents.map((ev) => {
                ...
                return (
                  <div
                    key={ev.id}
                    className="bg-[#0E0C0A] border border-white/[0.05] p-4 flex items-start gap-4"
                  >
                      ...
                      <div className="text-[11px] text-[#3A3530] mt-0.5">
                        {ev.wholeDay
                          ...
                          : [
                              ...
                              ev.endTime &&
                                `→ ${new Date(ev.endTime).toLocaleString("hu-HU")}`,
                            ]
                              .filter(Boolean)
                              .join(" ")}
                      </div>
          ...
          >
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={deleting}
                className="flex-1 py-2.5 bg-red-500/80 text-[11px] tracking-[0.1em] uppercase text-white hover:bg-red-500 transition-all disabled:opacity-50"
              >
      ...
      {toast && (
        // Toast – változatlan, hagyd a meglévő Toast komponenst
        <div
          className={`fixed bottom-4 right-4 z-[200] flex items-center gap-3 px-4 py-3 border text-[13px] max-w-sm ${
            toast.type === "success"
              ? "bg-[#0E0C0A] border-[#C8A882]/30 text-[#D4C4B0]"
              : "bg-[#0E0C0A] border-red-500/30 text-red-400"
          }`}
        >
          <div
            className={`w-1.5 h-1.5 rounded-full shrink-0 ${toast.type === "success" ? "bg-[#C8A882]" : "bg-red-400"}`}
          />
