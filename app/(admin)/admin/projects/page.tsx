...
// Kompakt dátum badge az admin listára (sötét téma)
function AdminDateBadge({ events }: { events: { startTime: string | null }[] }) {
  const now  = new Date();
  const next = (events ?? [])
  ...
  return (
    <div className={`flex items-center gap-2 px-2.5 py-1.5 border shrink-0 ${isUrgent ? "border-[#C8A882]/40 bg-[#C8A882]/5" : "border-white/[0.06] bg-transparent"}`}>
      <div className="text-center min-w-[1.8rem]">
        <div className="text-[7px] tracking-[0.1em] uppercase text-[#5A5248]">{HU_MONTHS_SH[next.getMonth()]}</div>
        <div className="font-['Cormorant_Garamond'] text-[1.3rem] font-light leading-none" style={{ color: isUrgent ? "#C8A882" : "#5A5248" }}>
          {next.getDate()}
        </div>
      </div>
...
export default function AdminProjectsPage() {
  ...
  const stats = { total: projects.length, planning: projects.filter(p=>p.status==="PLANNING").length, inProgress: projects.filter(p=>p.status==="IN_PROGRESS").length, completed: projects.filter(p=>p.status==="COMPLETED").length };
  ...
  return (
    <div className="min-h-screen bg-[#0C0A08] text-[#D4C4B0]">
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-5 flex flex-col gap-4">
        ...
        {/* Táblázat desktop – dátum oszloppal */}
        <div className="hidden lg:block bg-[#0E0C0A] border border-white/[0.05] overflow-hidden">
          <div className="grid grid-cols-[1fr_130px_130px_110px_90px_100px_100px] items-center border-b border-white/[0.05] px-4 gap-3">
            {["Projekt","Ügyfél","Típus","Dátum","Státusz","Tevékenység","Műveletek"].map(h=>(
              <div key={h} className="py-3 text-[9px] tracking-[0.14em] uppercase text-[#3A3530]">{h}</div>
            ))}
          </div>
          {loading ? (
            <div className="flex items-center justify-center py-16 gap-3"><div className="w-4 h-4 border border-[#C8A882]/30 border-t-[#C8A882] rounded-full animate-spin"/><span className="text-[12px] text-[#3A3530]">Betöltés...</span></div>
          ) : projects.length === 0 ? (
            <div className="flex items-center justify-center py-16"><span className="text-[12px] text-[#3A3530]">Nincs találat</span></div>
          ) : projects.map(p => (
            <div key={p.id} className="grid grid-cols-[1fr_130px_130px_110px_90px_100px_100px] items-center px-4 gap-3 border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
              ...
              <div className="py-3.5">
                <AdminDateBadge events={p.calendarEvents ?? []} />
              </div>
              <div className="py-3.5">
      ...
      {showNewModal && (
        <NewProjectModal onClose={()=>setShowNewModal(false)} onCreated={()=>{ setShowNewModal(false); fetchProjects(); setToast({msg:"Projekt létrehozva",type:"success"}); }} />
      )}
