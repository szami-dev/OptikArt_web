...
const PROJECT_TYPES       = [{id:1,name:"Esküvő"},{id:2,name:"Portré"},{id:3,name:"Rendezvény"},{id:4,name:"Marketing"},{id:5,name:"Drón"},{id:6,name:"Egyéb"}];
...
function Toast({ msg, type, onClose }: { msg: string; type: "success"|"error"; onClose: ()=>void }) {
  ...
  return (
    <div className={`fixed bottom-4 right-4 z-[200] flex items-center gap-3 px-4 py-3 border text-[13px] ${type==="success"?"bg-[#0E0C0A] border-[#C8A882]/30 text-[#D4C4B0]":"bg-[#0E0C0A] border-red-500/30 text-red-400"}`}>
      ...
      {msg}<button onClick={onClose} className="ml-2 opacity-50 hover:opacity-100">✕</button>
    </div>
  );
}
...
function DarkField({ label, required, children }: { label:string; required?:boolean; children:React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label className="text-[10px] tracking-[0.14em] uppercase text-[#5A5248]">{label}{required&&<span className="text-[#C8A882]/60 ml-0.5">*</span>}</label>
      {children}
    </div>
  );
}

function AdminDateBadge({ eventDate, events }: { eventDate: string | null; events: { startTime: string | null }[] }) {
  const now = new Date();
  ...
  return (
    <div className={`flex items-center gap-2 px-2.5 py-1.5 border shrink-0 ${isUrgent?"border-[#C8A882]/40 bg-[#C8A882]/5":"border-white/[0.06] bg-transparent"}`}>
      <div className="text-center min-w-[1.8rem]">
        <div className="text-[7px] tracking-[0.1em] uppercase text-[#5A5248]">{HU_MONTHS_SH[date.getMonth()]}</div>
        <div className="font-['Cormorant_Garamond'] text-[1.3rem] font-light leading-none" style={{color:isPast?"#3A3530":isUrgent?"#C8A882":"#5A5248"}}>{date.getDate()}</div>
      </div>
      <div className="border-l border-white/[0.06] pl-2">
        {isToday ? (
          <div className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-[#34D399] animate-pulse"/><span className="text-[9px] text-[#34D399]">Ma</span></div>
        ) : isPast ? (
          <div className="text-[8px] tracking-[0.06em] uppercase text-[#3A3530]">Volt</div>
        ) : (
          <>
            <div className="font-['Cormorant_Garamond'] text-[1rem] font-light leading-none" style={{color:isUrgent?"#C8A882":"#3A3530"}}>{daysUntil}</div>
            <div className="text-[7px] tracking-[0.08em] uppercase text-[#3A3530]">nap</div>
          </>
        )}
        {primary && <div className="text-[6px] text-[#C8A882]/40 mt-0.5 tracking-widest">✦</div>}
      </div>
    </div>
  );
}
...
function NewProjectModal({ onClose, onCreated }: { onClose:()=>void; onCreated:()=>void }) {
  ...
  const filteredPackages = packages.filter(p => p.categoryId === ({1:1,2:2} as Record<number,number>)[typeId]);
  ...
  async function handleCreate() {
    ...
    try {
      const res = await fetch("/api/projects/admin/create", {
        ...
        body:JSON.stringify({ name:name.trim(), description:description.trim()||null, typeId, packageId, status, userId:userId||null, totalPrice:totalPrice?parseFloat(totalPrice):null }),
      });
      if (!res.ok) throw new Error((await res.json()).error);
      onCreated();
    } catch (e:any) { setError(e.message ?? "Hiba a létrehozásnál"); }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center" onClick={onClose}>
      ...
      <div className="relative z-10 w-full max-w-lg mx-4 bg-[#0E0C0A] border border-white/[0.08] max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06] sticky top-0 bg-[#0E0C0A] z-10">
          <div>
            <div className="flex items-center gap-2 mb-0.5"><div className="w-3 h-px bg-[#C8A882]/50"/><span className="text-[9px] tracking-[0.2em] uppercase text-[#C8A882]/50">Admin</span></div>
        ...
        <div className="px-5 py-5 flex flex-col gap-4">
          <DarkField label="Projekt neve" required><input value={name} onChange={e=>setName(e.target.value)} placeholder="Pl. Kovács Esküvő 2025" className={inputCls}/></DarkField>
          <div className="grid grid-cols-2 gap-3">
            <DarkField label="Típus">
              <select value={typeId} onChange={e=>setTypeId(parseInt(e.target.value))} className={inputCls}>
                {PROJECT_TYPES.map(t=><option key={t.id} value={t.id} style={{background:"#141210"}}>{t.name}</option>)}
              </select>
            </DarkField>
            <DarkField label="Státusz">
              <select value={status} onChange={e=>setStatus(e.target.value as ProjectStatus)} className={inputCls} style={{color:STATUS_META[status].color}}>
                {STATUSES.map(s=><option key={s} value={s} style={{background:"#141210",color:STATUS_META[s].color}}>{STATUS_META[s].label}</option>)}
              </select>
            </DarkField>
          </div>
          {hasPackages && (
            <DarkField label="Csomag">
              <select value={packageId??""} onChange={e=>setPackageId(e.target.value?parseInt(e.target.value):null)} className={inputCls}>
                ...
                {filteredPackages.map(p=><option key={p.id} value={p.id} style={{background:"#141210"}}>{p.name}{p.price?` – ${p.price.toLocaleString("hu-HU")} Ft`:""}</option>)}
              </select>
            </DarkField>
          )}
          <DarkField label="Ügyfél (opcionális)">
            <select value={userId??""} onChange={e=>setUserId(e.target.value?parseInt(e.target.value):null)} className={inputCls}>
              ...
              {users.filter(u=>(u as any).role!=="ADMIN").map(u=><option key={u.id} value={u.id} style={{background:"#141210"}}>{u.name??u.email}</option>)}
            </select>
          </DarkField>
          <DarkField label="Végösszeg (Ft, opcionális)">
            <input type="number" value={totalPrice} onChange={e=>setTotalPrice(e.target.value)} placeholder="Pl. 380000" className={inputCls} min="0"/>
          </DarkField>
          <DarkField label="Megjegyzés">
            <textarea value={description} onChange={e=>setDesc(e.target.value)} rows={3} placeholder="Belső megjegyzés..." className={`${inputCls} resize-none`}/>
          </DarkField>
    ...
    <div className="min-h-screen bg-[#0C0A08] text-[#D4C4B0]">
      <div className="border-b border-white/[0.05] px-4 sm:px-6 lg:px-8 py-5 lg:py-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-1"><div className="w-4 h-px bg-[#C8A882]/50"/><span className="text-[9px] tracking-[0.2em] uppercase text-[#C8A882]/50">Admin panel</span></div>
          ...
          <button onClick={()=>setShowNewModal(true)} className="flex items-center gap-2 bg-[#C8A882] text-[#0C0A08] text-[11px] tracking-[0.14em] uppercase px-4 py-2.5 hover:bg-[#D4B892] transition-colors font-medium whitespace-nowrap">
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-5 flex flex-col gap-4">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
          {[{label:"Összes",value:stats.total,color:"#C8A882"},{label:"Tervezés",value:stats.planning,color:"#C8A882"},{label:"Folyamatban",value:stats.inProgress,color:"#60A5FA"},{label:"Kész",value:stats.completed,color:"#34D399"}].map(s=>(
            <div key={s.label} className="bg-[#0E0C0A] border border-white/[0.05] px-3 sm:px-5 py-3 sm:py-4">
              ...
              <div className="font-['Cormorant_Garamond'] text-[1.6rem] sm:text-[2rem] font-light leading-none" style={{color:s.color}}>{loading?"—":s.value}</div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <div className="flex border border-white/[0.06] overflow-x-auto scrollbar-none">
            <button onClick={()=>setStatusFilter("ALL")} className={`px-3 py-2 text-[10px] tracking-[0.1em] uppercase transition-all whitespace-nowrap border-r border-white/[0.04] ${statusFilter==="ALL"?"bg-[#C8A882]/15 text-[#C8A882]":"text-[#3A3530] hover:text-[#5A5248]"}`}>Mind</button>
            {STATUSES.map(s=>(
        ...
        <div className="hidden lg:block bg-[#0E0C0A] border border-white/[0.05] overflow-hidden">
          <div className="grid grid-cols-[1fr_130px_130px_130px_90px_100px_100px] items-center border-b border-white/[0.05] px-4 gap-3">
            {["Projekt","Ügyfél","Típus","Munka dátuma","Státusz","Tevékenység","Műveletek"].map(h=>(
              <div key={h} className="py-3 text-[9px] tracking-[0.14em] uppercase text-[#3A3530]">{h}</div>
            ))}
          </div>
          {loading ? (
            <div className="flex items-center justify-center py-16 gap-3"><div className="w-4 h-4 border border-[#C8A882]/30 border-t-[#C8A882] rounded-full animate-spin"/></div>
          ) : projects.length === 0 ? (
            <div className="flex items-center justify-center py-16"><span className="text-[12px] text-[#3A3530]">Nincs találat</span></div>
          ) : projects.map(p=>(
            <div key={p.id} className="grid grid-cols-[1fr_130px_130px_130px_90px_100px_100px] items-center px-4 gap-3 border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
              ...
              <div className="py-3.5"><span className="text-[11px] text-[#5A5248]">{p.type?.name??"—"}</span></div>
              <div className="py-3.5">
                <AdminDateBadge eventDate={p.eventDate} events={p.calendarEvents??[]}/>
              </div>
              <div className="py-3.5">
                <select value={p.status??"PLANNING"} onChange={e=>quickStatusChange(p.id,e.target.value as ProjectStatus)}
                  ...
                  {STATUSES.map(s=><option key={s} value={s} style={{background:"#0E0C0A",color:STATUS_META[s].color}}>{STATUS_META[s].label}</option>)}
                </select>
              </div>
              ...
              <div className="py-3.5">
                <Link href={`/admin/projects/${p.id}`} className="block text-center py-1.5 border border-white/[0.08] text-[10px] tracking-[0.08em] uppercase text-[#5A5248] hover:text-[#C8A882] hover:border-[#C8A882]/30 transition-all">Megnyit</Link>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="lg:hidden flex flex-col gap-2">
          {loading ? (
            <div className="flex items-center justify-center py-12"><div className="w-4 h-4 border border-[#C8A882]/30 border-t-[#C8A882] rounded-full animate-spin"/></div>
          ) : projects.map(p=>(
            <div key={p.id} className="bg-[#0E0C0A] border border-white/[0.05] p-4">
              ...
              <div className="flex gap-2">
                ...
                <Link href={`/admin/projects/${p.id}`} className="px-4 py-2 bg-[#C8A882]/10 border border-[#C8A882]/20 text-[11px] tracking-[0.08em] uppercase text-[#C8A882] hover:bg-[#C8A882]/15 transition-all whitespace-nowrap">Megnyit →</Link>
              </div>
            </div>
          ))}
        </div>
      ...
      {showNewModal && (
        <NewProjectModal
          ...
          onCreated={()=>{ setShowNewModal(false); fetchProjects(); setToast({msg:"Projekt létrehozva",type:"success"}); }}
        />
      )}
