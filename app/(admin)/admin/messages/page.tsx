...
function timeAgo(iso: string) {
  ...
  if (m < 1) return "Most"; if (m < 60) return `${m}p`;
...
function AdminMessagesInner() {
  ...
  const [filter, setFilter] = useState<"all" | "user" | "guest" | "project" | "standalone" | "unread">("all");
  ...
  const load = useCallback(async () => {
    ...
    if (filter === "standalone") umsgs = umsgs.filter(m => m.project === null);
    ...
    if (activeGuestIdRef.current !== null) {
      const updated = (guestData.sessions ?? []).find(
        (s: GuestSession) => s.id === activeGuestIdRef.current
      );
      if (updated) setActiveGuest(updated);
    }
  }, [filter]);
  ...
  useEffect(() => {
    ...
    if (guestId && guestSessions.length > 0) {
      ...
      if (found) { setActiveGuest(found); setActiveUser(null); }
    }
  ...
  if (filter === "guest") {
    ...
    if (filter !== "project" && filter !== "standalone" && filter !== "unread") {
      guestSessions.forEach(g => listItems.push({ kind: "guest", data: g }));
    }
    ...
    <div className="h-screen bg-[#0C0A08] text-[#D4C4B0] flex flex-col overflow-hidden">
      ...
      <div className="border-b border-white/[0.05] px-4 sm:px-6 lg:px-10 py-4 flex items-center justify-between gap-4 flex-wrap shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-['Cormorant_Garamond'] text-[1.8rem] font-light text-white leading-none">Üzenetek</h1>
          {totalUnread > 0 && (
            <span className="text-[9px] bg-[#F87171]/15 text-[#F87171] border border-[#F87171]/20 px-2 py-0.5">{totalUnread} új</span>
          )}
        </div>
        <div className="flex border border-white/[0.06] flex-wrap">
          {([
            { key: "all",        label: "Mind"       },
            ...
            <button key={f.key}
              onClick={() => { setFilter(f.key); setActiveUser(null); setActiveGuest(null); }}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Bal: lista */}
        <div className={`flex flex-col border-r border-white/[0.05] overflow-y-auto ${hasActive ? "hidden lg:flex lg:w-[320px] shrink-0" : "flex-1"}`}>
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="w-4 h-4 border border-[#C8A882]/30 border-t-[#C8A882] rounded-full animate-spin" />
            </div>
          ) : listItems.length === 0 ? (
            <div className="text-center py-12"><p className="text-[12px] text-[#3A3530]">Nincs üzenet.</p></div>
          ) : listItems.map(item => {
            ...
            if (item.kind === "guest") {
              ...
              return (
                <div key={`guest-${g.id}`}
                  onClick={() => { setActiveGuest(g); setActiveUser(null); }}
                  ...
                  <div className="flex items-start justify-between gap-2 mb-1.5">
                    <div className="flex items-center gap-2 min-w-0">
                      {needsReply && <div className="w-1.5 h-1.5 rounded-full bg-[#FBBF24] shrink-0" />}
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5">
                          <div className="text-[12px] text-[#D4C4B0] font-medium truncate">{g.name}</div>
                          <span className="text-[8px] border border-[#FBBF24]/20 text-[#FBBF24]/60 px-1">vendég</span>
                        </div>
                        <div className="text-[10px] text-[#3A3530] truncate">{g.email}</div>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <div className="text-[10px] text-[#3A3530]">{timeAgo(g.updatedAt)}</div>
                      <div className="text-[9px] text-[#3A3530] mt-0.5">{g._count.messages} üzenet</div>
                    </div>
                  </div>
                  {lastMsg && <p className="text-[11px] text-[#5A5248] line-clamp-1 ml-9">{lastMsg.body}</p>}
                  {needsReply && <div className="ml-9 mt-1 text-[9px] text-[#FBBF24]/60">Válaszra vár</div>}
                </div>
              );
            }
            ...
            return (
              <div key={`${msg._type ?? "chat"}-${msg.id}`}
                onClick={() => { setActiveUser(msg); setActiveGuest(null); if (isUnread) markUserRead(msg.id); }}
                className={`px-4 py-4 border-b border-white/[0.03] cursor-pointer transition-colors ${isActive ? "bg-[#C8A882]/6 border-l-2 border-l-[#C8A882]" : "hover:bg-white/[0.02]"}`}>
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="px-5 py-3.5 border-b border-white/[0.05] flex items-start justify-between gap-4 shrink-0">
              ...
              {activeUser._type !== "project_message" && (
                <button onClick={() => handleUserDelete(activeUser.id, activeUser._type)}
              ...
              {activeUser._type !== "project_message" && activeUser.replies.map(r => (
                <div key={r.id} className={`flex gap-3 ${r.sender.role === "ADMIN" ? "flex-row-reverse" : ""}`}>
                  <div className={`w-7 h-7 border flex items-center justify-center font-['Cormorant_Garamond'] text-[11px] shrink-0 mt-0.5 ${r.sender.role === "ADMIN" ? "border-[#C8A882]/40 text-[#C8A882] bg-[#C8A882]/8" : "border-white/[0.1] text-[#5A5248]"}`}>
                    {r.sender.role === "ADMIN" ? "A" : r.sender.name?.charAt(0).toUpperCase() ?? "?"}
                  </div>
                  <div className="flex-1">
                    <div className={`flex items-center gap-2 mb-1 ${r.sender.role === "ADMIN" ? "justify-end" : ""}`}>
                      <span className="text-[10px] text-[#3A3530]">{timeAgo(r.createdAt)}</span>
                      <span className="text-[11px] text-[#5A5248]">{r.sender.role === "ADMIN" ? "Admin" : r.sender.name}</span>
                    </div>
                    <div className={`border px-4 py-3 text-[13px] leading-relaxed ${r.sender.role === "ADMIN" ? "bg-[#C8A882]/8 border-[#C8A882]/20 text-[#D4C4B0]" : "bg-[#141210] border-white/[0.05] text-[#D4C4B0]"}`}>
                      {r.body}
                    </div>
                  </div>
                </div>
            ...
            {activeUser._type !== "project_message" && (
              <div className="shrink-0 border-t border-white/[0.05] p-4 flex gap-3">
                <textarea value={reply} onChange={e => setReply(e.target.value)}
                  ...
                  rows={3} placeholder="Válasz... (Enter = küldés, Shift+Enter = új sor)"
                ...
                <button onClick={handleUserReply} disabled={!reply.trim() || sending}
        ...
        {activeGuest && (
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-3">
              {activeGuest.messages.map(msg => (
                <div key={msg.id} className={`flex gap-3 ${msg.isAdminReply ? "flex-row-reverse" : ""}`}>
                  <div className={`w-7 h-7 border flex items-center justify-center font-['Cormorant_Garamond'] text-[11px] shrink-0 mt-0.5 ${msg.isAdminReply ? "border-[#C8A882]/40 text-[#C8A882] bg-[#C8A882]/8" : "border-[#FBBF24]/20 text-[#FBBF24] bg-[#FBBF24]/5"}`}>
                    {msg.isAdminReply ? "A" : activeGuest.name.charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1">
                    <div className={`flex items-center gap-2 mb-1 ${msg.isAdminReply ? "justify-end" : ""}`}>
                      <span className="text-[10px] text-[#3A3530]">{timeAgo(msg.createdAt)}</span>
                      <span className="text-[11px] text-[#5A5248]">{msg.isAdminReply ? "Admin" : activeGuest.name}</span>
                    </div>
                    <div className={`border px-4 py-3 text-[13px] leading-relaxed ${msg.isAdminReply ? "bg-[#C8A882]/8 border-[#C8A882]/20 text-[#D4C4B0]" : "bg-[#141210] border-white/[0.05] text-[#D4C4B0]"}`}>
                      {msg.body}
                    </div>
                  </div>
                </div>
            ...
            <div className="shrink-0 border-t border-white/[0.05] p-4 flex gap-3">
              <textarea value={reply} onChange={e => setReply(e.target.value)}
                onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleGuestReply(); } }}
                rows={3} placeholder="Válasz... (emailen is értesítjük)"
                className="flex-1 bg-[#141210] border border-white/[0.08] text-[13px] text-[#D4C4B0] placeholder:text-[#3A3530] px-3 py-2.5 focus:outline-none focus:border-[#C8A882]/40 resize-none" />
              <button onClick={handleGuestReply} disabled={!reply.trim() || sending}
                className="px-4 bg-[#C8A882] text-[11px] tracking-[0.1em] uppercase text-[#0C0A08] font-medium hover:bg-[#D4B892] transition-all disabled:opacity-40 self-end py-3">
                {sending ? "..." : "Küld →"}
              </button>
            </div>
        ...
        {!hasActive && (
          <div className="hidden lg:flex flex-1 items-center justify-center">
            <div className="text-center">
              <svg viewBox="0 0 24 24" fill="none" stroke="#C8A882" strokeWidth="1" className="w-10 h-10 mx-auto mb-3 opacity-15">
